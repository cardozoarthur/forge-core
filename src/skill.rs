...
- In the interactive `forge` REPL, use `j`/`k` to move panel focus, `enter` to open the focused panel, `m` to cycle compact/detailed/focus display modes, and `t` to cycle themes. Use `/cockpit`, `/task-board`, `/readiness`, `/schedules`, `/addons`, `/sessions`, `/harness`, `/logs`, `/permissions`, `/identity` and `/workflow-dag` to render operational panels in place. Use `/context --workflow <id> --task <task-id> --budget 1200 --strict` for bounded context inspection and `/handoff --workflow <id> --task <task-id> --executor codex` only after approving lease acquisition.
