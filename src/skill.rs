...
- Before reading historical memory for a task, inspect the `memory_policy` object returned by `forge context` or task handoff, or call `forge memory policy --project-root <project-root> --output json` / MCP `forge.memory.policy` when operating on a project. It carries the workflow/tenant-derived memory level, allowed scopes, tenant boundary, default audience and governed `forge memory search --workflow <workflow-id>` command; project policy also reports `.forge/memory-governance.json` status and effective defaults. Do not inline broad memory into executor prompts.
