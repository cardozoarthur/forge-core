...
- Typing only `forge` must open the advanced guided cockpit by default, similar to opencode/gemini entrypoints. Read `dashboard.guided_cockpit_panel`, call `forge interactive guided-cockpit --output json`, or use MCP `forge.interactive.guided_cockpit` for `forge.interactive.guided_cockpit.v1`; it exposes four panes, eight end-to-end operator steps, current-step detection, preview commands, confirmation gates, rollback hints and safe-action policy without mutating state.
