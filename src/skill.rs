...
- Use `forge interactive patch-workbench` or MCP `forge.interactive.patch_workbench` to inspect the current repo's bounded inline `diff_preview`, `diff_review_queue` and `edit_intake` before presenting a TUI/web review panel; `edit_intake` declares required inputs, missing fields, form readiness and safe command templates for `patch plan/review/diff/apply/revert/restore`. Then use `forge patch diff` or MCP tool `forge.patch.diff` to build a read-only multi-file diff navigation model with selectable file/hunk indexes before asking for human approval.
