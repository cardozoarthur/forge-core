...
- In `forge.context.request` / `forge.task.handoff`, treat `prompt_packet.organization_context` and `prompt_packet.personality_decision` as required executor context. `organization_context` carries organization, brand, product, user, channel, memory/personality scope, tenant policy, brand voice/tone/values, terminology, design-system sources and operating policy. `personality_decision` carries the Forge-owned routing owner, selected persona mode/profile, selected voice/tone, brand voice/tone/values, style sources, fallback mode and audit flag. Their checksums plus `organization_context_required` and `personality_decision_required` validation gates make agent output accountable to organizational context and personality routing.
