...
- In `forge.context.request` / `forge.task.handoff`, treat `prompt_packet.organization_context` as required executor context. It carries organization, brand, product, user, channel, memory/personality scope, tenant policy, brand voice/tone/values, terminology, design-system sources and operating policy, with `organization_context_sha256` and `organization_context_required` in validation gates so agent output should not ignore organizational context.
