...
- Read `dashboard.improvement_loop_panel`, call `forge interactive improvement-loop --output json`, or use MCP `forge.interactive.improvement_loop` before self-improvement or workflow mutation. It exposes `forge.interactive.improvement_loop.v1` with ranked candidates, critical/high counts, parallel-ready handoffs, avoidable AI cost, final-outcome gaps, stale/attention signals, structured-log counts, cost totals, validation failures, context quality and governed next commands without applying mutations.
- Read `dashboard.workflow_mutation_panel`, call `forge interactive workflow-mutation --output json`, or use MCP `forge.interactive.workflow_mutation` before replanning a live workflow. It exposes `forge.interactive.workflow_mutation.v1` with DAG, task-board, modifier lane proposals, handoffs, costs, events and safe `workflow update-goal`, `workflow update-node-brain` and `workflow attach-artifact` commands without applying mutations from the read-only panel.
- Typing only `forge` must open the OpenCode-style orchestrator-first Forge TUI by default. Normal input talks to the Forge orchestrator, which decides whether to answer directly or create a Forge workflow. Plan, build, agents and subagents are Forge workflows or workflow nodes; each node may have its own agent/brain routing. Use `forge tui --output json` for the same compact entrypoint model in scripts; it exposes orchestrator prompt operation, `!<cmd>` local shell execution, `!` shell-mode toggle, workflow/agent/subagent/node-agent/event/Addon/cost/handoff/approval capabilities and quick commands while keeping the legacy detailed cockpit under `forge interactive guided-cockpit --output json` or MCP `forge.interactive.guided_cockpit`.
- Read `dashboard.core_boundary_panel`, call `forge interactive core-boundary --project-root <project-root> --output json`, or use MCP `forge.interactive.core_boundary` with `project_root` for `forge.interactive.core_boundary.v1` before claiming Core/Addons architectural alignment. It audits universal Core responsibilities, domain-specific Core leaks, Addon-owned capability boundaries, visible compatibility executors and goal3 acceptance gates without mutating state.
...
- `forge request step` and `forge.run.step` never stand in for task execution. Every ready task requires the executor to perform the real work and close it through `forge request complete-task ... --evidence-command "<passing gate>" --evidence-exit-code <observed-exit-code>`; the exit code must be the value actually observed by the caller, and Forge never fabricates `0`. Without that receipt Forge returns `handoff_required` instead of promoting metadata as work.
- When an AI or mixed executor has actually done the ready handoff work, close it with `forge request complete-task --run <run-id> --task <task-id> --executor codex --summary "<result>" --evidence-command "<passing gate>" --evidence-exit-code <observed-exit-code> --origin codex --output json` or `forge.run.complete_task`; Forge writes a replayable execution trace, builds the executor response, validates it, promotes the task and immediately drives the next action.
...
- AWS is not a Forge Core runtime, build, installation, backup, or production-gate dependency. Only when an operator explicitly selects the optional AWS adoption integration, use `forge aws check`, `forge aws inventory` or MCP tools `forge.aws.check`, `forge.aws.inventory`, `forge.aws.raw`. These commands delegate to `~/plugins/aws-ops/scripts/aws-ops`, use the AWS credential-vault defaults and keep mutation gating in the aws-ops wrapper.
...
- Read `dashboard.addon_capability_panel`, call `forge interactive addon-capabilities --project-root <project-root> --output json`, or use MCP `forge.interactive.addon_capabilities` with `project_root` before rendering Addon/capability operations. It exposes `forge.interactive.addon_capability.v1` lifecycle counts, capability registry totals, permission gates, runtime contracts, TUI views and dispatch state without mutating workflows.
- Read `dashboard.schedule_panel`, call `forge interactive schedules --output json`, or use MCP `forge.interactive.schedules` before rendering scheduled workflow operations. It exposes `forge.interactive.schedules.v1` due workflows, scheduler worker capacity, deterministic assignment queues, sleep/backpressure/cancellation state and observed scheduled workflow rows without mutating workflows.
...
forge request complete-task --run <run-id> --task <task-id> --executor codex --summary "executor finished the ready task with passing evidence" --evidence-command "<passing gate>" --evidence-exit-code <observed-exit-code> --origin codex --output json
...
forge interactive ui-composition --project-root . --output json
forge mcp call forge.interactive.guided_cockpit --output json
forge mcp call forge.interactive.ui_composition --input '{"project_root":"."}' --output json
forge interactive core-boundary --project-root . --output json
forge mcp call forge.interactive.core_boundary --input '{"project_root":"."}' --output json
...
forge mcp call forge.interactive.schedules --output json
forge interactive context-memory --project-root . --output json
forge mcp call forge.interactive.context_memory --input '{"project_root":"."}' --output json
forge interactive operating-context --project-root . --output json
forge mcp call forge.interactive.operating_context --input '{"project_root":"."}' --output json
forge interactive architecture --project-root . --output json
forge mcp call forge.interactive.architecture --input '{"project_root":"."}' --output json
forge interactive improvement-loop --output json
forge mcp call forge.interactive.improvement_loop --output json
forge interactive workflow-mutation --output json
forge mcp call forge.interactive.workflow_mutation --output json
forge interactive addon-capabilities --project-root . --output json
forge mcp call forge.interactive.addon_capabilities --input '{"project_root":"."}' --output json
...
forge harness activation-profile --executor codex --shim-dir <dir> --project-root <project-root> --output json
forge harness activation-profile --executor codex --shim-dir <dir> --project-root <project-root> --shell-rc <path> --apply --approved-by <operator> --output json
forge mcp call forge.harness.activation_profile --input '{"executor":"codex","shim_dir":"<dir>","project_root":"<project-root>"}' --output json
...
forge harness exec --executor codex --forge-first --project-root <project-root> --workflow <workflow-id> --task <task-id> --run <run-id> --output json -- <cmd>
forge harness exec --executor codex --forge-first --project-root <project-root> --workflow <workflow-id> --task <task-id> --run <run-id> --execute --allow-exec --output json -- <cmd>
...
forge request complete-task --run <run-id> --task <task-id> --executor codex --summary "<validated evidence>" --evidence-command "<passing gate>" --evidence-exit-code <observed-exit-code> --origin codex --output json
