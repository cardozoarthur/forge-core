...
- Treat Codex, OpenCode, Gemini CLI, Claude CLI and future CLIs as replaceable execution brains only. Forge owns and routes workflow state, memory, skills, MCP servers/tools, credential-vault references, context packets, shell/session lifecycle, permissions, cost policy, validation gates and self-improvement decisions. Inspect this boundary with `forge brains --output json` or MCP `forge.brain_router`, inspect provider/session state with `forge sessions --output json` or MCP `forge.sessions`, and record shell session lifecycle with `forge sessions lifecycle` or MCP `forge.session.lifecycle`, before handing work to a brain.
