...
- Use `forge interactive patch-workbench` or MCP `forge.interactive.patch_workbench` to inspect the current repo's bounded inline `diff_preview`, `diff_review_queue`, `edit_intake` and ordered `operation_plan` before presenting a TUI/web review panel; `edit_intake` declares required inputs, missing fields, form readiness and safe command templates for `patch plan/review/diff/apply/revert/restore`, while `operation_plan` sequences the lifecycle steps, dependencies and human-approval gates. Then use `forge patch diff` or MCP tool `forge.patch.diff` to build a read-only multi-file diff navigation model with selectable file/hunk indexes before asking for human approval.
...
forge mcp call forge.interactive.release_gates --input '{"version":"0.5"}' --output json
