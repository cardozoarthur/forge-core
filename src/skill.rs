...
- `forge request step` and `forge.run.step` never stand in for task execution. A ready command, wait, or notification task requires the executor to perform the real work and close it through `forge request complete-task ... --evidence-command "<passing gate>"`; without that receipt Forge returns `handoff_required` instead of promoting metadata as work.
