...
pub struct CliHarnessExecReceipt {
    ...
    pub secret_injections: Vec<CliHarnessSecretInjection>,
...
#[derive(Debug, Clone, Serialize)]
pub struct CliHarnessSecretInjection {
    pub env: String,
...
pub struct CliHarnessExecOptions<'a> {
    ...
    pub secret_permissions: &'a [String],
    pub project_root: Option<&'a Path>,
...
pub fn run_cli_harness_exec(options: CliHarnessExecOptions<'_>) -> Result<CliHarnessExecReceipt> {
    ...
    safety_checks.push(format!(
        "native runtime security guardrails evaluated: {}",
        runtime_security_guardrails.decision
    ));
    ...
    let mut resolved_secret_env = Vec::<(String, String)>::new();
    let mut secret_injections = Vec::<CliHarnessSecretInjection>::new();
    if !secret_env.is_empty() {
        let secret_permission_allowed = secret_permissions_authorize(secret_permissions);
        if !secret_permission_allowed {
            for mapping in secret_env {
                let (env_name, vault_reference) = parse_secret_env_mapping(mapping)?;
                if let Some(store) = store {
                    let _ = store.resolve_runtime_secret(RuntimeSecretVaultAccess {
                ...
                secret_injections.push(CliHarnessSecretInjection {
                    env: env_name,
                    vault_reference,
            ...
            let mut receipt = exec_receipt(CliExecReceiptInput {
                ...
                resolved_executable,
                resolution_status,
                status: "harness_exec_blocked_secret_permission".to_string(),
                safety_checks,
                executed: false,
                success: None,
                exit_code: None,
        ...
        for mapping in secret_env {
            let (env_name, vault_reference) = parse_secret_env_mapping(mapping)?;
            let resolved = store.resolve_runtime_secret(RuntimeSecretVaultAccess {
                vault_reference: &vault_reference,
                workflow_id,
                requester: executor,
                allowed: true,
            ...
            secret_injections.push(CliHarnessSecretInjection {
                env: env_name,
                vault_reference: resolved.vault_reference,
                status: "allowed".to_string(),
                value_sha256: resolved.value_sha256,
                value_len: resolved.value_len,
                audit_event_id: Some(resolved.audit_event_id),
                redaction: "secret_value_redacted".to_string(),
            });
        }
    }
...
fn secret_permissions_authorize(secret_permissions: &[String]) -> bool {
    let has_credential_secret = secret_permissions
        .iter()
        .any(|permission| permission == "credential_secret_permissions");
    let has_tool_usage = secret_permissions
        .iter()
        .any(|permission| permission == "tool_usage_permissions");
    has_credential_secret && has_tool_usage
}

fn parse_secret_env_mapping(mapping: &str) -> Result<(String, String)> {
    let Some((env_name, vault_reference)) = mapping.split_once('=') else {
        bail!("secret env mapping must use ENV=vault.reference syntax");
    };
    let env_name = env_name.trim();
    let vault_reference = vault_reference.trim();
    if env_name.is_empty() || vault_reference.is_empty() {
        bail!("secret env mapping must include both env name and vault reference");
    }
    if !env_name
        .chars()
        .all(|ch| ch.is_ascii_uppercase() || ch.is_ascii_digit() || ch == '_')
    {
        bail!("secret env name must use uppercase ASCII, digits or underscore");
    }
    Ok((env_name.to_string(), vault_reference.to_string()))
}
...
struct CliExecReceiptInput {
    ...
    require_lineage_for_exec: bool,
    runtime_security_guardrails: RuntimeSecurityGuardrailReport,
...
fn exec_receipt(input: CliExecReceiptInput) -> CliHarnessExecReceipt {
    ...
    CliHarnessExecReceipt {
        ...
        runtime_security_guardrails: input.runtime_security_guardrails,
        ...
        secret_injections: input.secret_injections,
