...
fn git_repository(path: &Path) -> (String, String) {
    ...
    fs::write(
        &cargo,
        "#!/bin/sh\ncase \"${1:-}\" in\n  slow) sleep 2 ;;\n  fail) exit 7 ;;\n  test|--version) printf '%s\\n' 'forge mission executor fixture' ;;\n  *) exit 2 ;;\nesac\n",
    )
    .unwrap();
    ...
    assert!(Command::new("git")
        .args([
            ...
            path.to_str().unwrap(),
            "-c",
            "user.name=Forge Mission Test",
            "-c",
            "user.email=forge-mission@example.invalid",
            "commit",
            "--allow-empty",
            "-m",
            "initial",
...
fn rehash_receipt(receipt: &mut forge_core::mission_executor::MissionExecutionReceipt) {
...
fn install_real_host_toolchain(fixture: &Fixture) -> String {
    ...
    let config_path = fixture.repository.join(".forge/worktree.toml");
    let mut config: toml::Value =
        toml::from_str(&fs::read_to_string(&config_path).unwrap()).unwrap();
...
fn persist_mission(store: &ForgeStore, mission: &MissionRecord) {
    Connection::open(store.path())
        .unwrap()
        .execute(
...
fn compile_only_cargo_test_never_produces_a_tests_passed_claim() {
    ...
    forged.validations.push("tests_passed".to_string());
    let error = verified_mission_execution_claims(
        &forged,
        &fixture.mission_id,
        &fixture.workflow_id,
        forged.mission_revision,
        &fixture.task_id,
        &fixture.agent_id,
    )
    .unwrap_err();
...
fn active_execution_lease_survives_same_key_retry_and_blocks_other_keys() {
    ...
    let execution = thread::spawn(move || {
        let store = ForgeStore::open(execution_store).unwrap();
    ...
    let retry_store = ForgeStore::open(&store_path).unwrap();
    ...
    let competing_store = ForgeStore::open(&store_path).unwrap();
...
fn stale_receipt_remains_unconsumed_and_fresh_retry_survives_reopen() {
    ...
    let reopened = ForgeStore::open(fixture.store.path()).unwrap();
    ...
    let retry_store = ForgeStore::open(fixture.store.path()).unwrap();
