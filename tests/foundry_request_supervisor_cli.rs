...
use foundry_core::request::{create_run_record, load_run_record, save_run_record};
...
fn request_supervisor_cli_reports_an_empty_store() {
    ...
    assert_eq!(report["schema_version"], "foundry.request_supervisor.v1");
...
fn request_mcp_mutators_enforce_live_supervisor_lease_and_keep_normal_paths() {
    ...
    for (tool, run_id) in [
        ("foundry.request.cancel", &live_cancel),
        ("foundry.run.resume", &live_resume),
    ] {
