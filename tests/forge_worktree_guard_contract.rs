...
use forge_core::graph::TaskStatus;
...
const GUARDED_GOAL: &str = "Prepare an approved update to .forge/worktree.toml with auditable \
evidence.";
...
fn blocked_json(command: &mut Command) -> Value {
    ...
    assert!(
        !output.status.success(),
        "blocked forge command unexpectedly succeeded\nstdout:\n{}",
        String::from_utf8_lossy(&output.stdout)
    );
    serde_json::from_slice(&output.stdout).unwrap_or_else(|error| {
        panic!(
            "blocked forge output should be JSON: {error}\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        )
    })
...
fn init_repository(path: &Path) {
    ...
    for (key, value) in [
        ("user.email", "forge-worktree-guard@example.invalid"),
        ("user.name", "Forge Worktree Guard Test"),
    ] {
...
impl GuardFixture {
    ...
    fn write_guard_config_without_approval(&self) {
        fs::write(
            self.worktree_root.join(".forge/worktree.toml"),
            r#"schema_version = "forge.worktree.config.v1"

[guardrails]
modifiable_paths = ["src/", "tests/safe.rs"]
protected_paths = ["src/secret/", ".forge/worktree.toml"]
...
"#,
    ...
    fn approve_guard_config(&self, worktree_id: &str) {
        let approved = success_json(forge().arg("--store").arg(&self.store).args([
            "worktree",
            "approve-config",
            "--worktree",
            worktree_id,
...
fn guard_defaults_and_path_rules_allow_files_and_directories_with_protection_precedence() {
    ...
    let initialized = success_json(forge().arg("--store").arg(&fixture.store).args([
        "worktree",
        "inspect",
        "--worktree",
        &fixture.worktree_id,
    ...
    assert!(protected_defaults
        .iter()
        .any(|path| path == ".forge/worktree.toml"));
    ...
    for blocked_path in ["src/secret/key.rs", ".forge/worktree.toml", "README.md"] {
...
fn predecessor_creation_rejects_mixed_delegable_and_unsafe_paths_atomically() {
    let fixture = GuardFixture::new();
    fixture.write_guard_config();
    let before_store = ForgeStore::open(&fixture.store).unwrap();
    ...
    let output = command_output(forge().arg("--store").arg(&fixture.store).args([
        "worktree",
        "guard",
        "create-predecessor",
        "--worktree",
        ...
        "../outside",
        "--goal",
        "Update .forge/worktree.toml and ../outside with validated evidence",
...
fn blocked_guard_can_create_an_approved_predecessor_without_completing_current_task() {
    let fixture = GuardFixture::new();
    fixture.write_guard_config();
    let guarded_path = ".forge/worktree.toml";
    ...
    let denied = command_output(forge().arg("--store").arg(&fixture.store).args([
        "worktree",
        "guard",
        "create-predecessor",
        "--worktree",
    ...
    let unchanged_store = ForgeStore::open(&fixture.store).unwrap();
    ...
    let created = success_json(forge().arg("--store").arg(&fixture.store).args([
        "worktree",
        "guard",
        "create-predecessor",
        "--worktree",
...
fn retrying_predecessor_creation_reuses_the_incomplete_equivalent_task() {
    ...
    let first = success_json(forge().arg("--store").arg(&fixture.store).args([
        "worktree",
        "guard",
        "create-predecessor",
        "--worktree",
    ...
    let retried = success_json(forge().arg("--store").arg(&fixture.store).args([
        "worktree",
        "guard",
        "create-predecessor",
        "--worktree",
...
fn validated_predecessor_completion_reactivates_the_guarded_task() {
    ...
    let blocked_store = ForgeStore::open(&fixture.store).unwrap();
...
fn task_specific_binding_blocks_guard_checks_against_the_workflow_default_worktree() {
    ...
    fs::write(
        second_root.join(".forge/worktree.toml"),
        fs::read_to_string(fixture.worktree_root.join(".forge/worktree.toml")).unwrap(),
    )
    .unwrap();
    ...
    let wrong_protected =
        blocked_json(&mut fixture.guard_check(".forge/worktree.toml", Some("task-001")));
...
fn invalid_worktree_toml_is_an_explicit_failure_instead_of_falling_back_to_defaults() {
    let fixture = GuardFixture::new();
    let manifest = fixture.worktree_root.join(".forge/worktree.toml");
    fs::write(
        &manifest,
...
fn cli_integration_module_rename_keeps_the_legacy_harness_alias_without_the_old_file() {
    let new_type = std::any::type_name::<forge_core::cli_integration::CliHarnessExecReceipt>();
    let legacy_type = std::any::type_name::<forge_core::harness::CliHarnessExecReceipt>();
    assert_eq!(new_type, legacy_type);
    assert_eq!(
        forge_core::cli_integration::CLI_HARNESS_EXEC_SCHEMA_VERSION,
        forge_core::harness::CLI_HARNESS_EXEC_SCHEMA_VERSION
    );
    ...
    let compatibility = success_json(forge().args(["harness", "mode", "--output", "json"]));
    assert_eq!(compatibility["schema_version"], "forge.harness.mode.v1");
}
