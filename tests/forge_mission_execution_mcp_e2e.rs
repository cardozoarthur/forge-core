use forge_core::executor::{sync_executors, ExecutorSyncOptions};
...
fn git_repository(path: &Path) {
    ...
    assert!(Command::new("git")
        .args([
            ...
            path.to_str().unwrap(),
            "-c",
            "user.name=Forge Mission MCP E2E",
            "-c",
            "user.email=forge-mission-mcp@example.invalid",
            "commit",
            "--allow-empty",
            "-m",
            "initial",
...
fn mcp_mission_execution_is_receipt_backed_and_resumes_after_store_reopen() {
    ...
    let submit_tool = manifest
        .tools
        .iter()
        .find(|tool| tool.name == "forge.mission.submit")
        .unwrap();
    ...
    let execute_tool = manifest
        .tools
        .iter()
        .find(|tool| tool.name == "forge.mission.execute")
        .unwrap();
    ...
    let failed = call_mcp_tool(
        ...
        json!({
            ...
            "purpose": "test",
            "command": [cargo_command.clone(), "--definitely-invalid-forge-test"],
    ...
    let protected =
        call_mcp_tool(&store, "forge.mission.execute", execution_input.clone()).unwrap_err();
    ...
    let executed = call_mcp_tool(&store, "forge.mission.execute", execution_input.clone()).unwrap();
    ...
    let replayed = call_mcp_tool(&store, "forge.mission.execute", execution_input).unwrap();
    ...
    let forged = call_mcp_tool(
        ...
        json!({
            ...
            "task_id": task_id,
            "agent_id": agent_id,
            "idempotency_key": "mcp-forged-validation",
            "receipt_id": receipt_id,
            "summary": "forged validation must be rejected",
            "validations": ["requirements_summary"],
        }),
    )
    .unwrap_err();
    assert!(format!("{forged:#}").contains("unknown field `validations`"));
