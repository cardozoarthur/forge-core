...
use foundry_core::request::start_async_request_with_project_and_idempotency;
...
fn initialize_store(path: &Path) {
    drop(FoundryStore::open(path).unwrap());
}
...
fn cli_and_mcp_expose_idempotent_request_start() {
    ...
    let mcp_store = FoundryStore::open(&mcp_store_path).unwrap();
...
fn detached_cli_replay_launches_exactly_one_logical_driver() {
    ...
    let first_output = foundry().args(args).output().unwrap();
    assert!(
        first_output.status.success(),
        "{}",
        String::from_utf8_lossy(&first_output.stderr)
    ...
    let replay_output = foundry().args(args).output().unwrap();
    assert!(
        replay_output.status.success(),
        "{}",
        String::from_utf8_lossy(&replay_output.stderr)
