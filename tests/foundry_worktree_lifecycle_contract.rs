...
fn failure_json(command: &mut Command) -> Value {
    ...
    serde_json::from_slice(&output.stdout).unwrap_or_else(|error| {
        panic!(
            "failed foundry command should return a JSON contract: {error}\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        )
    })
...
fn init_repository(path: &Path) {
    ...
    for (key, value) in [
        ("user.email", "foundry-worktree-lifecycle@example.invalid"),
        ("user.name", "Foundry Worktree Lifecycle Test"),
    ] {
...
impl LifecycleFixture {
    ...
    fn config_path(&self) -> PathBuf {
        self.repository.join(".foundry/worktree.toml")
    }
    ...
    fn approve_config_output(&self) -> Output {
        command_output(foundry().arg("--store").arg(&self.store).args([
            "worktree",
            "approve-config",
            "--worktree",
            &self.worktree_id,
    ...
    fn sandbox_start(&self, allow_exec: bool, command: &[&str]) -> Command {
        ...
        if allow_exec {
            foundry.arg("--allow-exec");
        }
        foundry.args(["--output", "json", "--"]).args(command);
        foundry
    }
...
fn timeout_covers_pipe_holding_descendants_and_kills_the_process_group() {
    ...
    let pid_path = fixture
        .repository
        .join(".foundry/sandboxes/internal/artifacts/descendant.pid");
    let command = format!(
...
fn timeout_kills_a_setsid_daemon_that_keeps_captured_pipes_open() {
    ...
    let pid_path = fixture
        .repository
        .join(".foundry/sandboxes/internal/artifacts/setsid-pipes.pids");
    let command = format!(
...
fn launch_failure_returns_and_persists_a_structured_sandbox_receipt() {
    let fixture = LifecycleFixture::new();
    let missing = "foundry-contract-command-that-does-not-exist";
...
fn inherited_environment_blocks_a_secret_even_when_the_variable_name_is_neutral() {
    let fixture = LifecycleFixture::new();
    let environment_name = "FOUNDRY_BUILD_LABEL";
...
fn preview_start_status_stop_is_persistent_authorized_and_kills_the_group() {
    ...
    let leader_path = fixture
        .repository
        .join(".foundry/sandboxes/internal/artifacts/preview-leader.pid");
    let child_path = fixture
        .repository
        .join(".foundry/sandboxes/internal/artifacts/preview-child.pid");
    let command = format!(
    ...
    let status = success_json(foundry().arg("--store").arg(&fixture.store).args([
        "worktree",
        "sandbox",
        "status",
        SANDBOX_SELECTOR_FLAG,
    ...
    let denied_stop = command_output(foundry().arg("--store").arg(&fixture.store).args([
        "worktree",
        "sandbox",
        "stop",
        SANDBOX_SELECTOR_FLAG,
    ...
    let still_running = success_json(foundry().arg("--store").arg(&fixture.store).args([
        "worktree",
        "sandbox",
        "status",
        SANDBOX_SELECTOR_FLAG,
    ...
    let stopped = success_json(foundry().arg("--store").arg(&fixture.store).args([
        "worktree",
        "sandbox",
        "stop",
        SANDBOX_SELECTOR_FLAG,
    ...
    let final_status = success_json(foundry().arg("--store").arg(&fixture.store).args([
        "worktree",
        "sandbox",
        "status",
        SANDBOX_SELECTOR_FLAG,
...
fn persistent_stop_kills_a_redirected_setsid_descendant() {
    ...
    let pid_path = fixture
        .repository
        .join(".foundry/sandboxes/internal/artifacts/setsid-redirected.pid");
    let command = format!(
...
fn status_reconciles_a_sigkilled_supervisor_and_kills_payload_descendants() {
    ...
    let child_path = fixture
        .repository
        .join(".foundry/sandboxes/internal/artifacts/supervisor-loss-child.pid");
    let command = format!(
...
fn mcp_sandbox_plan_run_start_status_and_stop_match_the_cli_contracts() {
    ...
    let mcp_plan_envelope = fixture.mcp_call("foundry.worktree.sandbox.plan", common_input.clone());
    ...
    let mcp_run_envelope = fixture.mcp_call("foundry.worktree.sandbox.run", run_input);
    ...
    let cli_status = success_json(foundry().arg("--store").arg(&fixture.store).args([
        "worktree",
        "sandbox",
        "status",
        SANDBOX_SELECTOR_FLAG,
...
fn mcp_rejects_task_scoped_sandboxes_without_a_workflow_id() {
    ...
    for tool in [
        ...
        if tool != "foundry.worktree.sandbox.plan" {
