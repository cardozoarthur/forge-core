...
fn initialize_repository(repository: &Path) {
    ...
    git(
        repository,
        &["config", "user.name", "Foundry Teamwork Stress Tests"],
    );
    fs::write(repository.join("README.md"), "teamwork stress fixture\n").unwrap();
...
fn save_ready_executor(store: &FoundryStore, executor: &str) {
    let state = ExecutorState {
        ...
        config_evidence: vec!["teamwork stress fixture".to_string()],
        non_interactive_ready: true,
        probe_evidence: vec!["teamwork stress fixture probe".to_string()],
        foundry_first_ready: false,
        foundry_first_entrypoint: None,
        harness_status: None,
        allowed: true,
        decision_source: "teamwork_stress_test".to_string(),
    ...
    store
        .save_executor_quota(
            ...
            &serde_json::json!({
                ...
                "suitability": "teamwork stress handoff",
                "source": "teamwork_stress_test",
...
fn test_stress_cognitive_tasks_halting() {
    ...
    let workflow_id = json["teamwork"]["workflow_id"].as_str().unwrap();
...
fn test_stress_benchmark_cache_and_url_fetch() {
    ...
    let output_unreachable = foundry()
        .arg("--store")
        .arg(store_path.to_str().unwrap())
        .args([
            "teamwork",
        ...
        .env("FOUNDRY_BENCHMARK_URL", "http://127.0.0.1:9999/benchmarks")
        .assert()
        .success()
        .get_output()
        .stdout
    ...
    // With the remote benchmark unreachable, Foundry falls back to the first allowed modern brain.
    ...
    let output_malformed = foundry()
        ...
        .env("FOUNDRY_BENCHMARK_URL", &malformed_server.url)
        .assert()
        .success()
        .get_output()
        .stdout
