...
#[cfg(unix)]
use foundry_core::milestone::ProductionMissionLifecycleReceipt;
#[cfg(unix)]
...
fn foundry_json(store: &Path, args: &[&str]) -> Value {
    ...
    assert!(
        ...
        "foundry command failed: args={args:?} status={} policy_trace={}",
        output.status,
...
fn git_repository(path: &Path) {
    ...
    fs::write(
        ...
        concat!(
            "[package]\n",
            "name = \"foundry-production-lifecycle-cli-fixture\"\n",
            "version = \"0.1.0\"\n",
            "edition = \"2021\"\n",
            "\n",
            "[lib]\n",
    ...
    let commit = ProcessCommand::new("git")
        .args([
            ...
            path.to_str().unwrap(),
            "-c",
            "user.name=Foundry Production Evidence",
            "-c",
            "user.email=foundry-production-evidence@example.invalid",
            "commit",
            "--allow-empty",
            "-m",
            "initial",
...
fn production_mission_evidence_writes_exact_typed_artifact_bytes() {
    ...
    assert_eq!(
        ...
        "foundry.milestone.mission_lifecycle_evidence_package.v1"
    );
