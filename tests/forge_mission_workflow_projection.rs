...
fn forge_json_failure(store: &Path, args: &[&str]) -> Value {
    json_stdout(
        forge()
            .arg("--store")
            .arg(store)
...
fn git_repository(path: &Path) {
    ...
    let commit = ProcessCommand::new("git")
        .args([
            ...
            path.to_str().unwrap(),
            "-c",
            "user.name=Forge Mission Workflow Projection",
            "-c",
            "user.email=forge-mission-workflow-projection@example.invalid",
            "commit",
            "--allow-empty",
            "-m",
            "initial",
...
fn blocked_validation(fixture: &MissionFixture, workflow_id: &str) -> Value {
    let validation = forge_json_failure(
        &fixture.store,
