...
fn forge_json(store: &Path, args: &[&str]) -> Value {
    json_stdout(forge().arg("--store").arg(store).args(args).assert())
}
...
fn git_repository(path: &Path) {
    ...
    let commit = ProcessCommand::new("git")
        .args([
            ...
            path.to_str().unwrap(),
            "-c",
            "user.name=Forge Mission Lifecycle",
            "-c",
            "user.email=forge-mission-lifecycle@example.invalid",
            "commit",
            "--allow-empty",
            "-m",
            "initial",
...
fn execute_receipt(
    ...
    let executed = forge_json(&fixture.store, &args);
...
fn operational_gate_failure_repairs_and_revalidates_before_completion() {
    ...
    let intake_consumed = forge_json(
        &fixture.store,
        &["mission", "resume", mission_id, "--output", "json"],
    );
    ...
    let delivery_consumed = forge_json(
        &fixture.store,
        &["mission", "resume", mission_id, "--output", "json"],
    );
    assert_eq!(delivery_consumed["action"], "handoff_consumed");

    let review = forge_json(
        &fixture.store,
        &["mission", "drive", mission_id, "--output", "json"],
    );
...
fn custom_squad_with_five_gates_completes_without_a_repair_loop() {
    ...
    let delivery_resumed = forge_json(
        &fixture.store,
        &["mission", "resume", mission_id, "--output", "json"],
    );
