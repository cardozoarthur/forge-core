...
use forge_core::executor_runtime::{
...
use forge_core::worktree::{bind_worktree, create_worktree, WorktreeCreateOptions};
...
fn wave_overlaps_three_agy_and_five_codex_workers_in_eight_task_worktrees() {
    ...
    let handle = thread::spawn(move || {
        let store = ForgeStore::open(wave_store_path).unwrap();
...
fn concurrent_same_lease_executes_one_child_and_replays_one_receipt() {
    ...
    let first = thread::spawn(move || {
        let store = ForgeStore::open(first_store_path).unwrap();
        execute_executor_runtime(&store, first_request)
    });
    let second_store_path = store_path.clone();
    let second = thread::spawn(move || {
        let store = ForgeStore::open(second_store_path).unwrap();
...
fn initialize_repository(repository: &Path) {
    ...
    git(
        repository,
        &["config", "user.email", "forge-runtime@example.invalid"],
    );
    git(repository, &["config", "user.name", "Forge Runtime Test"]);
...
printf '%s\n' "$@" > "$marker_dir/args/$FORGE_TASK_ID"
cat > "$marker_dir/stdin/$FORGE_TASK_ID"
if ! mkdir "$marker_dir/invocations/$FORGE_TASK_LEASE_ID" 2>/dev/null; then
  : > "$marker_dir/duplicate-invocation"
fi
: > "$marker_dir/started/$FORGE_TASK_ID"
while [ ! -f "$marker_dir/release" ]; do
  sleep 0.02
done
printf '{"executor":"%s","task":"%s"}\n' "$name" "$FORGE_TASK_ID"
printf 'stderr-%s-%s\n' "$name" "$FORGE_TASK_ID" >&2
"#,
    )
    .unwrap();
...
git -c user.name='Forge Runtime Executor' \
    -c user.email='forge-runtime-executor@example.invalid' \
