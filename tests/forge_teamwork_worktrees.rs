...
use forge_core::worktree::{discover_worktrees, list_registered_worktrees};
...
fn initialize_repository(repository: &Path) {
    ...
    git(repository, &["config", "user.name", "Forge Teamwork Tests"]);
...
fn plan_teamwork(store: &ForgeStore) -> String {
...
fn options(
    ...
    TeamworkWorktreePrepareOptions {
        ...
        branch_prefix: "forge/teamwork-test".to_string(),
...
fn cli_prepare_teamwork(
    ...
    let mut command = AssertCommand::cargo_bin("forge").expect("forge binary should build");
    command
        .arg("--store")
        .arg(store_path)
...
fn stored_workflow_ids(store: &ForgeStore) -> BTreeSet<String> {
    store
        .load_workflows()
        .unwrap()
        .into_iter()
...
fn stored_run_ids(store: &ForgeStore) -> BTreeSet<String> {
    store
        .load_runs()
        .unwrap()
        .into_iter()
...
fn worktree_dry_run_is_read_only_and_returns_explicit_commands_for_all_agentic_tasks() {
    ...
    assert!(report.commands.iter().all(|command| {
        command.first().map(String::as_str) == Some("forge")
