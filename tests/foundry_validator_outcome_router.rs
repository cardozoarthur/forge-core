...
use foundry_core::storage::{FoundryStore, RuntimeContractDispatchWrite};
...
fn setup_workflow(store: &FoundryStore) -> (String, String) {
...
fn seed_validator_dispatch(
    ...
    let input = json!({
        "schema_version": "foundry.addon_validator_dispatch_input.v1",
    ...
    store
        .save_runtime_contract_dispatch(RuntimeContractDispatchWrite {
            id: dispatch_id,
            addon_id: "foundry.addon.validator-test",
...
) -> foundry_core::addon::AddonValidatorOutcomeApplicationReport {
...
fn cli_and_mcp_expose_the_same_validator_outcome_contract() {
    ...
    let cli_store = cli_temp.path().join("foundry.sqlite");
    ...
    let mcp_store = mcp_temp.path().join("foundry.sqlite");
    ...
    let tool = manifest["tools"]
        ...
        .unwrap()
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.apply_validator_outcome")
        .unwrap();
    assert_eq!(
        tool["output_schema"],
        ADDON_VALIDATOR_OUTCOME_APPLICATION_SCHEMA_VERSION
