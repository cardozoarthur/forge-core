...
fn initialize_repository(repository: &Path) {
    ...
    git(
        repository,
        &["config", "user.name", "Foundry Challenger Tests"],
    );
...
fn test_challenger_cognitive_task_handoff_halts() {
    ...
    prepare_teamwork_worktrees(
        ...
        TeamworkWorktreePrepareOptions {
            ...
            repository,
            worktree_root,
            branch_prefix: "foundry/challenger-test".to_string(),
    ...
    for _ in 0..15 {
        let step_output = Command::cargo_bin("foundry")
            .unwrap()
            .arg("--store")
            .arg(store_path.to_str().unwrap())
            .args([
...
fn test_challenger_all_brains_denied_policy_bypass() {
    ...
    let assert_result = Command::cargo_bin("foundry")
        .unwrap()
        .arg("--store")
        .arg(store_path.to_str().unwrap())
        .args([
...
fn test_challenger_expired_cache_without_url_drops_scores() {
    ...
    // Run without FOUNDRY_BENCHMARK_URL set.
    let output = Command::cargo_bin("foundry")
        .unwrap()
        .arg("--store")
        .arg(store_path.to_str().unwrap())
...
fn test_challenger_https_benchmark_url_ignored_silently() {
    ...
    let output = Command::cargo_bin("foundry")
        ...
        .env("FOUNDRY_BENCHMARK_URL", "https://127.0.0.1:0/benchmarks")
        .assert()
        .success()
        .get_output()
        .stdout
