...
use foundry_core::graph::{self, ExecutorKind, TaskStatus, Workflow};
...
fn open_store() -> (TempDir, FoundryStore) {
    let temporary = tempfile::tempdir().unwrap();
    let store = FoundryStore::open(temporary.path().join("foundry.sqlite")).unwrap();
    (temporary, store)
}

fn workflow_with_tasks(store: &FoundryStore, executors: &[ExecutorKind]) -> Workflow {
