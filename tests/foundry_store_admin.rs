...
fn backup_restore_round_trip_is_consistent_and_keeps_rollback() {
    ...
    let store_path = directory.path().join("foundry.sqlite");
    let backup_path = directory.path().join("foundry.backup.sqlite");
...
fn backup_is_private_by_default() {
    ...
    let backup_path = backup_directory.join("foundry.sqlite");
    Connection::open(&store_path).expect("source");
...
fn checkpointed_wal_check_and_backup_work_on_read_only_mount_with_encoded_path() {
    ...
    let store_path = source_directory.join("foundry ?#%.sqlite");
    let backup_path = backup_directory.join("foundry.sqlite");
    ...
    let check = run_foundry_store_command(&store_path, &["check"], None, &backup_directory);
    ...
    let backup = run_foundry_store_command(
        &store_path,
        &["backup"],
        Some(&backup_path),
        &backup_directory,
...
fn wal_without_shm_fails_closed_on_read_only_mount() {
    ...
    fs::create_dir_all(&backup_directory).expect("backup directory");
    let live_store = live_directory.join("foundry.sqlite");
    let fixture_store = fixture_directory.join("foundry.sqlite");
    ...
    let check = run_foundry_store_command(&fixture_store, &["check"], None, &backup_directory);
    assert!(
