...
fn git_repository(path: &Path) {
    ...
    let commit = ProcessCommand::new("git")
        .args([
            ...
            path.to_str().unwrap(),
            "-c",
            "user.name=Forge Mission CLI E2E",
            "-c",
            "user.email=forge-mission-cli@example.invalid",
            "commit",
            "--allow-empty",
            "-m",
            "initial",
...
fn json_command(store: &Path, args: &[&str], succeeds: bool) -> Value {
    let assertion = forge().arg("--store").arg(store).args(args).assert();
