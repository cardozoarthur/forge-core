...
use foundry_core::addon::{
    builtin_addon_catalog, resolve_goal_capabilities, CAP_PARALLEL_TEAMWORK,
};
use foundry_core::event::{ingest_inbound_event, route_inbound_event, InboundEventIngestInput};
use foundry_core::graph::{self, CoreOrchestrationSpec, ExecutorKind, TaskStatus, Workflow};
...
fn seed_workflow(store: &FoundryStore, status: &str) -> Workflow {
    let mut workflow =
...
fn core_orchestration_defaults_survive_legacy_deserialization() {
    ...
    assert_eq!(restored.core_orchestration.authority, "foundry_core");
...
fn parallel_teamwork_is_a_universal_core_capability() {
    ...
    assert_eq!(capability.source_addon, "foundry.core.kernel");
...
fn structural_validation_rejects_corrupt_policy_duplicate_edges_and_cycles() {
    ...
    assert!(messages.contains("authority must be foundry_core"));
...
fn cli_and_mcp_expose_the_same_core_mutations() {
    ...
    assert_eq!(mcp.result["schema_version"], "foundry.workflow_mutation.v1");
