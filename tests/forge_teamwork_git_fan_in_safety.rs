...
fn initialize_repository(repository: &Path) {
    ...
    git(
        repository,
        &["config", "user.name", "Forge Git Fan-In Safety Tests"],
    );
...
fn fixture() -> Fixture {
    ...
    let report = prepare_teamwork_worktrees(
        ...
        TeamworkWorktreePrepareOptions {
            ...
            branch_prefix: "forge/git-fan-in-safety".to_string(),
...
fn rebinding_a_source_to_another_worktree_makes_the_receipt_stale() {
    ...
    let alternate = create_worktree(
        ...
        WorktreeCreateOptions {
            ...
            path: alternate_path,
            branch: "forge/git-fan-in-safety-rebound-source".to_string(),
...
fn destination_lock_rejects_a_concurrent_fan_in_without_mutation_and_retry_succeeds() {
    ...
    let lock_file = OpenOptions::new()
        ...
        .read(true)
        .write(true)
        .open(git_dir.join("forge-dependency-fan-in.lock"))
        .unwrap();
...
fn external_git_driver_is_rejected_and_commit_hooks_remain_disabled() {
    ...
    git(
        &fixture.repository,
        &["config", "--unset-all", "merge.forge-unsafe.driver"],
    );
...
fn poisoned_git_redirection_environment_cannot_redirect_cli_fan_in() {
    ...
    let output = AssertCommand::cargo_bin("forge")
        .unwrap()
        .arg("--store")
