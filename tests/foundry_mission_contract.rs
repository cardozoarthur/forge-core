...
const FOUNDRY_ORIGINALS: [(&str, &str); 16] = [
...
fn squad_catalog_exposes_restricted_versioned_original() {
    ...
    assert_eq!(json["schema_version"], "foundry.squad.catalog.v1");
    let squads = json["squads"].as_array().unwrap();
    assert_eq!(squads.len(), FOUNDRY_ORIGINALS.len());
    for (squad, (expected_id, expected_name)) in squads.iter().zip(FOUNDRY_ORIGINALS) {
        assert_eq!(squad["id"], expected_id);
        assert_eq!(squad["name"], expected_name);
        assert_eq!(squad["version"], "1.0.0");
        assert_eq!(squad["distribution"]["origin"], "foundry-original");
        ...
        assert_eq!(
            ...
            format!("foundry-original:{expected_id}:1.0.0")
        );
...
fn originals_install_list_inspect_and_clone_as_immutable_versions() {
    ...
    assert_eq!(install_reports.len(), FOUNDRY_ORIGINALS.len());
    ...
    assert_eq!(installed.len(), FOUNDRY_ORIGINALS.len());
    let installed_ids = installed
        .iter()
        .map(|squad| squad["id"].as_str().unwrap())
        .collect::<std::collections::BTreeSet<_>>();
    let expected_ids = FOUNDRY_ORIGINALS
        .iter()
        .map(|(id, _)| *id)
        .collect::<std::collections::BTreeSet<_>>();
    assert_eq!(installed_ids, expected_ids);

    let original_inspect = foundry()
        .arg("--store")
        .arg(&store)
        .args([
            "squad",
    ...
    assert_eq!(
        ...
        "foundry-original:release-squad:1.0.0"
    );
...
fn bounded_mission_simulation_persists_composition_handoffs_gates_and_repair() {
    ...
    let output = foundry()
        ...
        .clone();
    let json: serde_json::Value = serde_json::from_slice(&output).unwrap();
    assert_eq!(json["schema_version"], "foundry.mission.simulation.v1");
