...
use forge_core::worktree::{bound_worktree_context, inspect_registered_worktree};
...
fn initialize_repository(repository: &Path) {
    ...
    git(
        repository,
        &["config", "user.name", "Forge Git Fan-In Tests"],
    );
...
fn fixture(frontend_agents: usize, backend_agents: usize) -> Fixture {
    ...
    let report = prepare_teamwork_worktrees(
        ...
        TeamworkWorktreePrepareOptions {
            ...
            repository,
            worktree_root,
            branch_prefix: "forge/git-fan-in-test".to_string(),
