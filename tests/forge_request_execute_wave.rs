...
use forge_core::graph::{create_workflow, task, ExecutorKind, NodeBrainAgentSlotSpec, TaskStatus};
use forge_core::intent::parse_intent;
use forge_core::request::{create_run_record, drive_request_with_context_budget, save_run_record};
...
use forge_core::teamwork_fan_in::{
...
fn execute_wave_requires_explicit_process_authorization_before_drive() {
    ...
    forge()
        .arg("--store")
        .arg(temp.path().join("forge.sqlite"))
        .args([
            "request",
            "execute-wave",
            "--run",
...
fn execute_wave_runs_admitted_task_and_keeps_promotion_separate() {
    ...
    assert_eq!(report["schema_version"], "forge.request_executor_wave.v1");
...
fn execute_wave_progresses_three_agy_five_codex_then_joins_and_auditor() {
    ...
    for worker_task_id in &fixture.branch_task_ids {
        assert!(
            auditor_root
                .join(format!("forge-runtime-{worker_task_id}.txt"))
                .is_file(),
    ...
    for join_task_id in &join_task_ids {
        assert!(auditor_root
            .join(format!("forge-runtime-{join_task_id}.txt"))
            .is_file());
    }
}
...
fn seed_parallel_teamwork_run(
    ...
    let preparation = prepare_teamwork_worktrees(
        ...
        TeamworkWorktreePrepareOptions {
            ...
            branch_prefix: "forge/request-wave-test".to_string(),
...
fn execute_parallel_wave_with_skip(
    ...
    command.env("FORGE_TEST_HOST_RESOURCE_SNAPSHOT_JSON", resource_snapshot);
    if let Some(task_id) = skip_commit_task {
        command.env("FORGE_TEST_SKIP_COMMIT_TASK", task_id);
    }
    let output = command
        .arg("--store")
        .arg(store_path)
...
fn assert_branches_remain_pending(store: &ForgeStore, fixture: &ParallelTeamworkFixture) {
...
fn save_ready_executor(store: &ForgeStore, executor: &str, command_path: &Path) {
...
printf '%s\n' "$@" > "$marker_dir/parallel-invocations/$FORGE_TASK_ID.args"
cat > "$marker_dir/parallel-invocations/$FORGE_TASK_ID.stdin"
if [ "${FORGE_TEST_SKIP_COMMIT_TASK:-}" != "$FORGE_TASK_ID" ]; then
  task_file="forge-runtime-$FORGE_TASK_ID.txt"
  printf 'committed by %s\n' "$FORGE_TASK_ID" > "$task_file"
  git add -- "$task_file"
  git commit -q -m "forge runtime $FORGE_TASK_ID"
fi
