...
use foundry_core::addon::load_addon_manifest_from_path;
...
case "$1" in
  resolve)
    ...
    [ "$no_newline" = "1" ] || exit 8
    printf '%s' "$FOUNDRY_FAKE_VAULT_SECRET"
    ;;
  *)
    ...
    let handle = thread::spawn(move || {
        for _ in 0..expected_requests {
            ...
            assert_eq!(
                ...
                "foundry.addon_runtime_worker_request.v1"
            );
    ...
    let handle = thread::spawn(move || {
        for _ in 0..expected_requests {
            ...
            let response_body = serde_json::json!({
                "status": "completed",
                "result": {
                    "schema_version": "foundry.addon_planning_strategy_result.v1",
    ...
    let handle = thread::spawn(move || {
        for _ in 0..expected_requests {
            ...
            assert_eq!(
                ...
                "foundry.event_egress_request.v1"
            );
            assert_eq!(event_request["addon_id"], "foundry.addon.partner");
        ...
        for _ in 0..expected_requests {
            ...
            let timestamp = headers
                ...
                .find_map(|line| {
                    let (name, value) = line.split_once(':')?;
                    name.trim()
                        .eq_ignore_ascii_case("X-Foundry-Timestamp")
                        .then(|| value.trim().to_string())
                })
            ...
            let nonce = headers
                ...
                .find_map(|line| {
                    let (name, value) = line.split_once(':')?;
                    name.trim()
                        .eq_ignore_ascii_case("X-Foundry-Nonce")
                        .then(|| value.trim().to_string())
                })
            ...
            assert_eq!(event_request["secret_env"], "FOUNDRY_TEST_EGRESS_SECRET");
...
fn addon_package_payload_bytes(
    ...
    serde_json::to_vec(&serde_json::json!({
        "schema_version": "foundry.addon_package.v1",
...
fn addons_catalog_exposes_core_kernel_and_first_party_addons() {
    ...
    assert_eq!(json["schema_version"], "foundry.addon_catalog.v1");
    ...
    assert!(json["addons"]
        ...
        .unwrap()
        .iter()
        .any(|addon| addon["id"] == "foundry.core.kernel"));
    ...
    assert!(core["context_providers"]
        ...
        .iter()
        .any(
            |provider| provider["id"] == "foundry.core.operating_context"
                && provider["scopes"]
                    .as_array()
                    .unwrap()
                    .iter()
    ...
    assert!(core["memory_providers"]
        ...
        .unwrap()
        .iter()
        .any(|provider| provider["id"] == "foundry.core.file_memory"
    ...
    assert!(core["event_adapters"]
        ...
        .unwrap()
        .iter()
        .any(|adapter| adapter["id"] == "foundry.core.event_inbox"
            && adapter["transport"] == "foundry_inbox"
            && adapter["direction"] == "ingress"));
    assert!(json["addons"]
        .as_array()
        .unwrap()
        .iter()
        .any(|addon| addon["id"] == "foundry.addon.visual_workspace"));
    let workflow_automation = json["addons"]
        .as_array()
        .unwrap()
        .iter()
        .find(|addon| addon["id"] == "foundry.addon.workflow_automation")
        .unwrap();
    assert!(workflow_automation["runtime_contracts"]
        ...
        .any(
            |contract| contract["id"] == "n8n_primitive_research.planning_strategy"
                ...
                && contract["runtime"] == "foundry_core_builtin"
        ));
    let visual_workspace = json["addons"]
        .as_array()
        .unwrap()
        .iter()
        .find(|addon| addon["id"] == "foundry.addon.visual_workspace")
        .unwrap();
    ...
    let software_development = json["addons"]
        ...
        .unwrap()
        .iter()
        .find(|addon| addon["id"] == "foundry.addon.software_development")
        .expect(
    ...
    let multimodal = json["addons"]
        ...
        .unwrap()
        .iter()
        .find(|addon| addon["id"] == "foundry.addon.multimodal")
    ...
    assert!(multimodal["runtime_contracts"]
        ...
        .any(
            |contract| contract["id"] == "multimodal_runtime_benchmark.executor"
                ...
                && contract["runtime"] == "foundry_core_builtin"
                && contract["entrypoint"] == "foundry.multimodal.runtime_benchmark"
                && contract["permissions"]
                    .as_array()
                    .unwrap()
...
fn mcp_manifest_exposes_addon_capability_tools() {
    ...
    let events = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.events.list")
        .expect("event stream MCP tool should be listed");
    assert_eq!(events["output_schema"], "foundry.event_stream.v1");
    ...
    for tool_name in [
        ...
        assert!(tool["output_schema"]
            .as_str()
            .unwrap()
            .starts_with("foundry.event_"));
    }
    let adapters = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.events.adapters")
        .expect("event adapter MCP tool should be listed");
    assert_eq!(adapters["output_schema"], "foundry.addon_event_adapters.v1");
    assert_eq!(adapters["mutates_workflow"], false);
    let contracts = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.contracts")
    ...
    let contract_policy = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.contract_policy")
    ...
    let dispatch_contract = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.dispatch_contract")
    ...
    let dispatch_planner = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.dispatch_planner")
    ...
    let dispatches = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.dispatches")
    ...
    let run_dispatch = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.run_dispatch")
    ...
    let execute_dispatch = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.execute_dispatch")
    ...
    let execute_validator = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.execute_validator")
    ...
    let execute_executor = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.execute_executor")
        .expect("addon executor execution MCP tool should be listed");
    assert_eq!(
        execute_executor["output_schema"],
        "foundry.addon_executor_execution.v1"
    );
    assert_eq!(execute_executor["mutates_workflow"], true);
    let execute_handoff = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.execute_handoff")
    ...
    let claim_dispatch = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.claim_dispatch")
    ...
    let complete_dispatch = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.complete_dispatch")
    ...
    let register_worker = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.register_worker")
    ...
    let workers = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.workers")
        .expect("addon runtime worker list MCP tool should be listed");
    assert_eq!(workers["output_schema"], "foundry.addon_runtime_workers.v1");
    assert_eq!(workers["mutates_workflow"], false);
    let views = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.views")
        .expect("addon views MCP tool should be listed");
    assert_eq!(views["output_schema"], "foundry.addon_views.v1");
    assert_eq!(views["mutates_workflow"], false);

    let identity = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.context")
    ...
    let identity_registry = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.registry")
    ...
    let tenant_index = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.tenant_index")
        .expect("tenant index MCP tool should be listed");
    assert_eq!(tenant_index["output_schema"], "foundry.tenant_index.v1");
    let tenant_audit = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.tenant_audit")
        .expect("tenant audit MCP tool should be listed");
    assert_eq!(tenant_audit["output_schema"], "foundry.tenant_audit.v1");
    let identity_memberships = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.memberships")
    ...
    let membership_update = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.membership_update")
    ...
    let identity_link = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.link")
        .expect("identity link MCP tool should be listed");
    assert_eq!(identity_link["output_schema"], "foundry.identity_link.v1");
    assert_eq!(identity_link["mutates_workflow"], true);
    let identity_unlink = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.unlink")
        .expect("identity unlink MCP tool should be listed");
    assert_eq!(identity_unlink["output_schema"], "foundry.identity_link.v1");
    assert_eq!(identity_unlink["mutates_workflow"], true);
    let identity_links = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.links")
        .expect("identity links MCP tool should be listed");
    assert_eq!(identity_links["output_schema"], "foundry.identity_links.v1");
    assert_eq!(identity_links["mutates_workflow"], false);
    let identity_resolve = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.resolve")
    ...
    let tenant_policy = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.tenant_policy")
        .expect("tenant policy MCP tool should be listed");
    assert_eq!(tenant_policy["output_schema"], "foundry.tenant_policy.v1");
    let identity_sync = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.identity.sync")
        .expect("identity sync MCP tool should be listed");
    assert_eq!(identity_sync["output_schema"], "foundry.identity_sync.v1");

    let catalog = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.catalog")
        .expect("addon catalog MCP tool should be listed");
    assert_eq!(catalog["output_schema"], "foundry.addon_catalog.v1");
    assert_eq!(catalog["mutates_workflow"], false);

    let resolve = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.resolve")
        .expect("addon resolver MCP tool should be listed");
    assert_eq!(resolve["output_schema"], "foundry.capability_resolution.v1");
    assert_eq!(
    ...
    let validate = tools
        .iter()
        .find(|tool| tool["name"] == "foundry.addons.validate")
        .expect("addon validation MCP tool should be listed");
    assert_eq!(validate["output_schema"], "foundry.addon_validation.v1");
...
fn software_patch_goals_resolve_through_first_party_addon_capability() {
    ...
    let json: Value = serde_json::from_slice(&output).unwrap();
    assert_eq!(json["schema_version"], "foundry.capability_resolution.v1");
    ...
    assert!(json["required_capabilities"]
        ...
        .any(
            |capability| capability["id"] == "source_code_patch_lifecycle"
                && capability["source_addon"] == "foundry.addon.software_development"
    ...
    assert!(json["active_addons"]
        .as_array()
        .unwrap()
        .contains(&serde_json::json!("foundry.addon.software_development")));
    assert!(json["runtime_contracts"]
        ...
        .any(
            |contract| contract["id"] == "source_code_patch_lifecycle.executor"
                && contract["source_addon"] == "foundry.addon.software_development"
                && contract["entrypoint"] == "foundry.patch.lifecycle"
        ));
}
...
fn multimodal_goals_resolve_through_first_party_addon_capability() {
    ...
    assert!(json["required_capabilities"]
        ...
        .iter()
        .any(|capability| capability["id"] == "multimodal_runtime"
            && capability["source_addon"] == "foundry.addon.multimodal"
            && capability["matched_keywords"]
                .as_array()
                .unwrap()
    ...
    assert!(json["active_addons"]
        .as_array()
        .unwrap()
        .contains(&serde_json::json!("foundry.addon.multimodal")));
    assert!(json["runtime_contracts"]
        .as_array()
        .unwrap()
        .iter()
        .any(
            |contract| contract["id"] == "multimodal_runtime_benchmark.executor"
                && contract["source_addon"] == "foundry.addon.multimodal"
                && contract["entrypoint"] == "foundry.multimodal.runtime_benchmark"
...
fn mcp_call_resolves_goal_capabilities_through_addon_registry() {
    ...
    let json: Value = serde_json::from_slice(&output).unwrap();
    assert_eq!(json["schema_version"], "foundry.mcp.call.v1");
    assert_eq!(json["status"], "ok");
    assert_eq!(json["tool_name"], "foundry.addons.resolve");
    let result = &json["result"];
    assert_eq!(result["schema_version"], "foundry.capability_resolution.v1");
    assert!(result["required_capabilities"]
        .as_array()
        .unwrap()
        .iter()
        .any(|capability| capability["id"] == "visual_workspace"
            && capability["source_addon"] == "foundry.addon.visual_workspace"));
}
...
"#,
    ...
    let suggestion = json["capability_suggestions"]
        ...
        .find(|suggestion| {
            suggestion["capability_id"] == "route_optimization"
                && suggestion["addon_id"] == "foundry.addon.route"
        })
    ...
    assert_eq!(suggestion["mcp_tools"][0], "foundry.addons.enable");
    assert_eq!(suggestion["commands"][0][0], "foundry");
    assert_eq!(suggestion["commands"][0][2], "enable");
    assert_eq!(suggestion["commands"][0][3], "foundry.addon.route");
}
...
"#,
    ...
    let repository = "registry://foundry/routes";
    let channel = "stable";
    let package_id = "foundry.addon.route@0.1.0";
    ...
    assert_eq!(suggestion["addon_id"], "foundry.addon.route");
    ...
    assert_eq!(suggestion["mcp_tools"][0], "foundry.addons.marketplace");
    assert_eq!(suggestion["mcp_tools"][1], "foundry.addons.install_package");
...
"#,
    ...
    let repository = "registry://foundry/routes";
    let channel = "stable";
    let package_id = "foundry.addon.route@0.4.0";
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&serde_json::json!({
            "schema_version": "foundry.addon_registry_index.v1",
            "packages": [
                {
...
"#,
    ...
    let repository = "registry://foundry/routes";
    let channel = "stable";
    let package_id = "foundry.addon.route@0.2.0";
    ...
    let remote_source = "https://example.com/foundry/routes/route-0.2.0.package.json";
    ...
    assert_eq!(suggestion["status"], "available_in_marketplace_package");
    assert_eq!(suggestion["mcp_tools"][0], "foundry.addons.marketplace");
    assert_eq!(suggestion["mcp_tools"][1], "foundry.addons.fetch_package");
...
fn first_party_extension_planners_prefer_capability_resolution_over_text_triggers() {
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.workflow_blueprint
    ...
    assert!(
        json["intent"]["capability_resolution"]["workflow_extensions"]
            ...
            .any(|extension| extension["id"] == "n8n_primitive_research"
                && extension["source_addon"] == "foundry.addon.workflow_blueprint")
    );
...
fn addon_planner_registry_exposes_core_and_external_planning_strategies() {
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.route_planner
...
"#,
    ...
    let output = foundry()
        ...
        .clone();
    let json: Value = serde_json::from_slice(&output).unwrap();
    assert_eq!(json["schema_version"], "foundry.addon_planner_registry.v1");
    ...
    assert_eq!(builtin["mcp_tools"][0], "foundry.addons.resolve");
    ...
    assert!(external["mcp_tools"]
        ...
        .contains(&Value::String(
            "foundry.addons.dispatch_planner".to_string()
        )));
    ...
    let mcp_dispatch_input = serde_json::json!({
        "addon_id": "foundry.addon.route_planner",
    ...
    let mcp_dispatch_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
...
fn external_planning_strategy_executes_through_worker_with_equivalence_audit() {
    ...
    fs::write(
        addon_dir.join("route-planner.yaml"),
        r#"
id: foundry.addon.route_planner_runtime
...
"#,
    ...
    let cli_output = foundry()
        .args([
            ...
            addon_dir.to_str().unwrap(),
            "--addon",
            "foundry.addon.route_planner_runtime",
            "--contract",
            "route.strategy.planning",
            "--worker",
            "route-planner-api-worker",
    ...
    let mcp_input = serde_json::json!({
        ...
        "addon_id": "foundry.addon.route_planner_runtime",
        "contract_id": "route.strategy.planning",
...
fn addon_validator_executes_through_worker_with_result_audit_for_cli_and_mcp() {
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.quality_validator
...
print(json.dumps({
    "status": "completed",
    "result": {
        "schema_version": "foundry.addon_validator_result.v1",
        "decision": "passed",
...
"#,
    ...
    let cli_output = foundry()
        .args([
            ...
            addon_dir.to_str().unwrap(),
            "--addon",
            "foundry.addon.quality_validator",
            "--contract",
    ...
    let mcp_input = serde_json::json!({
        "addon_dirs": [addon_dir.display().to_string()],
        "addon_id": "foundry.addon.quality_validator",
...
fn addon_executor_executes_through_worker_with_result_audit_for_cli_and_mcp() {
    ...
    fs::create_dir_all(&addon_dir).unwrap();
    fs::write(
        addon_dir.join("fulfillment-executor.yaml"),
        r#"
id: foundry.addon.fulfillment_executor
name: Fulfillment Executor Addon
version: 0.1.0
capabilities:
  - id: fulfillment_execution
    title: Fulfillment execution
    domains: [operations]
    keywords: [fulfillment-execution]
runtime_contracts:
  - id: fulfillment.task.executor
    title: Fulfillment task executor
    contract_type: executor
    capability_id: fulfillment_execution
    runtime: wasm
    entrypoint: fulfillment_task.run
    inputs: [task_ref, input, context]
    outputs: [status, outputs, artifacts, events]
"#,
    )
    .unwrap();

    let worker_script = temp.path().join("fulfillment-executor-worker.py");
    fs::write(
        &worker_script,
        r#"#!/usr/bin/env python3
import json
...
request = json.load(sys.stdin)
order_id = request["input"]["input"]["order_id"]
print(json.dumps({
    "status": "completed",
    "result": {
        "schema_version": "foundry.addon_executor_result.v1",
        "status": "completed",
        "task_ref": request["input"]["task_ref"],
        "summary": "fulfillment task completed",
        "outputs": {
            "order_id": order_id,
            "confirmation": "ok"
        },
        "artifacts": [
            {"kind": "receipt", "id": "receipt-" + order_id}
        ],
        "events": [
            {"kind": "fulfilled", "order_id": order_id}
        ],
        "context_tenant": request["input"]["context"]["provided_context"].get("tenant")
    },
    "attestation": {
...
"#,
    ...
    let worker_data = serde_json::json!({
        "execution_mode": "local_process",
        "command": worker_script.display().to_string(),
        "allowed_entrypoints": ["fulfillment_task.run"],
        "allowed_contracts": ["fulfillment.task.executor"],
        "timeout_seconds": 5
    })
    .to_string();
    foundry()
    ...
    let plan_output = foundry()
        .args([
            ...
            "plan",
            "--goal",
            "Audit fulfillment Addon artifacts inside a Foundry workflow",
            "--output",
            "json",
        ])
        .assert()
    ...
    let cli_output = foundry()
        .args([
            ...
            addon_dir.to_str().unwrap(),
            "--addon",
            "foundry.addon.fulfillment_executor",
            "--contract",
            "fulfillment.task.executor",
            "--worker",
            "fulfillment-executor-worker",
            "--task",
    ...
    assert_eq!(cli_json["status"], "addon_executor_completed");
    assert_eq!(
        cli_json["dispatch_report"]["status"],
        "runtime_contract_dispatch_external_completed"
    );
    ...
    assert_eq!(
        cli_json["dispatch_report"]["dispatches"][0]["input"]["task_ref"],
        "fulfillment-cli-task"
    );
    assert_eq!(cli_json["validation"]["status"], "valid");
    assert_eq!(cli_json["validation"]["result_status"], "completed");
    assert_eq!(cli_json["validation"]["completed"], true);
    assert_eq!(cli_json["validation"]["artifact_count"], 1);
    assert_eq!(cli_json["validation"]["event_count"], 1);
    assert_eq!(
        cli_json["executor_result"]["outputs"]["order_id"],
        "ord-cli-001"
    );
    ...
    let mcp_input = serde_json::json!({
        "addon_dirs": [addon_dir.display().to_string()],
        "addon_id": "foundry.addon.fulfillment_executor",
    ...
    assert_eq!(mcp_json["result"]["status"], "addon_executor_completed");
    assert_eq!(
...
fn addon_handoff_executes_through_worker_with_result_audit_for_cli_and_mcp() {
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.partner_handoff
...
print(json.dumps({
    "status": "completed",
    "result": {
        "schema_version": "foundry.addon_handoff_result.v1",
        "status": "delivered",
...
"#,
    ...
    let mcp_input = serde_json::json!({
        "addon_dirs": [addon_dir.display().to_string()],
        "addon_id": "foundry.addon.partner_handoff",
...
"#,
    ...
    let output = foundry()
        ...
        .clone();
    let json: Value = serde_json::from_slice(&output).unwrap();
    assert_eq!(json["schema_version"], "foundry.addon_observability.v1");
    ...
    assert_eq!(addon["addon_id"], "foundry.addon.telemetry");
    ...
    let mcp_input = serde_json::json!({
        "addon_id": "foundry.addon.telemetry",
        "addon_dirs": [addon_dir],
        "dispatch_limit": 50
    });
    let mcp_output = foundry()
...
fn event_stream_projects_legacy_events_into_tenant_aware_envelopes() {
    ...
    let events_json: Value = serde_json::from_slice(&events_output).unwrap();
    assert_eq!(events_json["schema_version"], "foundry.event_stream.v1");
...
fn event_envelopes_project_normalized_observability_metrics() {
    ...
    store_handle
        .record_event(
            ...
            &serde_json::json!({
                ...
                "task_id": "task-002",
                "addon_id": "foundry.addon.payment",
                "duration_ms": 245,
                "retry_count": "2",
                "wait_seconds": 30,
    ...
    assert_eq!(
        ...
        "foundry.event_observability.v1"
    );
    assert_eq!(observed["observability"]["node_ref"], "node-payment");
    assert_eq!(
        observed["observability"]["addon_id"],
        "foundry.addon.payment"
    );
    ...
    let observability_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
...
"#,
    ...
    let mcp_timeline_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
...
"#,
    ...
    store_handle
        .record_global_event(GlobalEventWrite {
            ...
            data: &serde_json::json!({
                ...
                "addon_id": "foundry.addon.visible",
                "duration_ms": 222,
    ...
    let mcp_observability_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
    ...
    store_handle
        .record_global_event(GlobalEventWrite {
            ...
            data: &serde_json::json!({
                ...
                "addon_id": "foundry.addon.history_visible",
                "duration_ms": 333,
                "retry_count": 1
            }),
            tenant_context: &tenant_context,
    ...
    store_handle
        .record_global_event(GlobalEventWrite {
            ...
            data: &serde_json::json!({
                ...
                "addon_id": "foundry.addon.history_hidden",
                "duration_ms": 999,
                "retry_count": 9
            }),
            tenant_context: &other_tenant_context,
        ...
        connection
            .execute_batch(
                ...
                VALUES (
                    ...
                    '{}',
                    '{"node_id":"legacy-node","addon_id":"foundry.addon.legacy","duration_ms":77,"retry_count":1,"wait_seconds":9,"state":"waiting_for_backfill","context":{"effective_budget":900,"routing_summary":{"selected_bytes":810,"remaining_budget":90},"memory_policy":{"memory_level":"short_term","memory_scope":"organization"}}}',
                ...
                VALUES (
                    ...
                    'legacy-node', 'foundry.addon.legacy', 77, 1, 'waiting_for_backfill', 9,
...
fn event_observability_history_rolls_up_time_buckets_for_cli_and_mcp() {
    ...
    let store = temp.path().join("foundry.sqlite");
    {
        let _store_handle = FoundryStore::open(&store).unwrap();
    }
    {
        let connection = Connection::open(&store).unwrap();
        connection
            .execute_batch(
                ...
                (
                    ...
                    'org-demo', 'brand-demo', 'product-demo', 'user-demo', 'web',
                    '{}',
                    '{"node_id":"node-stock","addon_id":"foundry.addon.inventory","duration_ms":120,"retry_count":0,"wait_seconds":5,"context":{"effective_budget":1000,"routing_summary":{"selected_bytes":500,"remaining_budget":500},"memory_policy":{"memory_level":"standard","memory_scope":"project"}}}',
                ...
                (
                    ...
                    'org-demo', 'brand-demo', 'product-demo', 'user-demo', 'web',
                    '{}',
                    '{"node_id":"node-stock","addon_id":"foundry.addon.inventory","duration_ms":180,"retry_count":1,"wait_seconds":7,"context":{"effective_budget":1000,"routing_summary":{"selected_bytes":850,"remaining_budget":150},"memory_policy":{"memory_level":"standard","memory_scope":"project"}}}',
                ...
                (
                    ...
                    'org-demo', 'brand-demo', 'product-demo', 'user-demo', 'web',
                    '{}',
                    '{"node_id":"node-payment","addon_id":"foundry.addon.payment","duration_ms":75,"retry_count":2,"wait_seconds":3,"context":{"effective_budget":800,"routing_summary":{"selected_bytes":760,"remaining_budget":40},"memory_policy":{"memory_level":"short_term","memory_scope":"organization"}}}',
    ...
    let inventory_first_day = buckets
        .iter()
        .find(|bucket| {
            bucket["group_id"] == "foundry.addon.inventory"
                && bucket["bucket_start"]
                    .as_str()
                    .unwrap()
    ...
    let payment_second_day = buckets
        .iter()
        .find(|bucket| {
            bucket["group_id"] == "foundry.addon.payment"
                && bucket["bucket_start"]
                    .as_str()
                    .unwrap()
    ...
    let mcp_input = serde_json::json!({
        ...
        "addon_id": "foundry.addon.inventory",
        "bucket": "day",
        "group_by": "addon"
    });
    let mcp_output = foundry()
        ...
        connection
            .execute_batch(
                ...
                (
                    ...
                    'org-demo', 'brand-demo', 'product-demo', 'user-demo', 'web',
                    '{}',
                    '{"node_id":"node-render","addon_id":"foundry.addon.content","duration_ms":500,"retry_count":0,"wait_seconds":5,"context":{"effective_budget":1000,"routing_summary":{"selected_bytes":950,"remaining_budget":50},"memory_policy":{"memory_level":"standard","memory_scope":"project"}}}',
                ...
                (
                    ...
                    'org-demo', 'brand-demo', 'product-demo', 'user-demo', 'web',
                    '{}',
                    '{"node_id":"node-render","addon_id":"foundry.addon.content","duration_ms":600,"retry_count":0,"wait_seconds":5,"context":{"effective_budget":1000,"routing_summary":{"selected_bytes":960,"remaining_budget":40},"memory_policy":{"memory_level":"standard","memory_scope":"project"}}}',
                ...
                (
                    ...
                    'org-demo', 'brand-demo', 'product-demo', 'user-demo', 'web',
                    '{}',
                    '{"node_id":"node-render","addon_id":"foundry.addon.content","duration_ms":700,"retry_count":0,"wait_seconds":5,"context":{"effective_budget":1000,"routing_summary":{"selected_bytes":940,"remaining_budget":60},"memory_policy":{"memory_level":"standard","memory_scope":"project"}}}',
                ...
                (
                    ...
                    'org-demo', 'brand-demo', 'product-demo', 'user-demo', 'web',
                    '{}',
                    '{"node_id":"node-payment","addon_id":"foundry.addon.billing","duration_ms":100,"retry_count":2,"wait_seconds":40,"context":{"effective_budget":1000,"routing_summary":{"selected_bytes":700,"remaining_budget":300},"memory_policy":{"memory_level":"short_term","memory_scope":"organization"}}}',
                ...
                (
                    ...
                    'org-demo', 'brand-demo', 'product-demo', 'user-demo', 'web',
                    '{}',
                    '{"node_id":"node-payment","addon_id":"foundry.addon.billing","duration_ms":100,"retry_count":2,"wait_seconds":40,"context":{"effective_budget":1000,"routing_summary":{"selected_bytes":720,"remaining_budget":280},"memory_policy":{"memory_level":"short_term","memory_scope":"organization"}}}',
        ...
        store_handle
            .record_global_event(GlobalEventWrite {
                ...
                data: &serde_json::json!({
                    ...
                    "addon_id": "foundry.addon.policy_visible",
                    "duration_ms": 75,
                    "retry_count": 0
                }),
                tenant_context: &tenant_context,
        ...
        store_handle
            .record_global_event(GlobalEventWrite {
                ...
                data: &serde_json::json!({
                    ...
                    "addon_id": "foundry.addon.policy_hidden",
                    "duration_ms": 900,
                    "retry_count": 0
                }),
                tenant_context: &other_tenant_context,
...
fn tenant_index_tracks_async_run_resources() {
    ...
    let tenant_index_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
    ...
    assert_eq!(
        tenant_index_json["schema_version"],
    ...
    assert_eq!(audit_json["schema_version"], "foundry.tenant_audit.v1");
    assert_eq!(audit_json["status"], "tenant_index_complete");
    assert_eq!(audit_json["missing_count"], 0);

    let mcp_audit_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
...
"#,
    ...
    let policy_output = foundry()
        ...
        .clone();
    let policy_json: Value = serde_json::from_slice(&policy_output).unwrap();
    assert_eq!(policy_json["schema_version"], "foundry.tenant_policy.v1");
...
"#,
    ...
    let mcp_context_stderr = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
...
"#,
    ...
    assert_eq!(ingest_json["schema_version"], "foundry.event_ingest.v1");
    ...
    let inbox_output = foundry()
        ...
        .clone();
    let inbox_json: Value = serde_json::from_slice(&inbox_output).unwrap();
    assert_eq!(inbox_json["schema_version"], "foundry.event_inbox.v1");
    assert_eq!(inbox_json["event_count"], 1);

    let global_timeline_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    assert_eq!(route_json["schema_version"], "foundry.event_route.v1");
    assert_eq!(route_json["status"], "event_routed");
    assert_eq!(
        route_json["adapter_policy"]["schema_version"],
        "foundry.event_adapter_policy.v1"
    );
    ...
    let workflow_timeline_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let scan_ingest_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    assert_eq!(scan_json["schema_version"], "foundry.event_worker.v1");
    ...
    let modify_ingest_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let modify_route_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let modified_events_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    .to_string();
    let attach_ingest_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let attach_route_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    .to_string();
    let checkpoint_ingest_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let checkpoint_route_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let continued_events_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let complete_ingest_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let complete_route_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
...
fn event_route_enforces_declared_addon_adapter_permission_policy() {
    let temp = tempdir().unwrap();
    let foundry_dir = temp.path().join(".foundry");
    let addon_dir = foundry_dir.join("addons");
    fs::create_dir_all(&addon_dir).unwrap();
    fs::write(
        foundry_dir.join("operating-context.yaml"),
        r#"
...
"#,
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.partner_events
name: Partner Events
version: 0.1.0
permissions:
...
"#,
    ...
    assert!(plan_json["intent"]["required_capabilities"]
        ...
        .iter()
        .any(|capability| capability["id"] == "route_optimization"
            && capability["source_addon"] == "foundry.addon.logistics"));
    assert!(plan_json["tasks"].as_array().unwrap().iter().any(|task| {
        task["title"] == "Apply Addon workflow extension: route optimization workflow"
            && task["context_requirements"]
                ...
                .unwrap()
                .iter()
                .any(|requirement| requirement == "source Addon foundry.addon.logistics")
            && task["execution_policy"]["reuse_hint"]
                == "addon_extension:route_optimization_workflow"
    }));

    let identity_output = foundry()
        .args([
            "identity",
            "context",
            "--project-root",
    ...
    assert_eq!(identity_json["context"]["brand"]["id"], "foundry");

    let sync_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
    ...
    assert_eq!(sync_json["schema_version"], "foundry.identity_sync.v1");
    ...
    assert_eq!(membership_json["memberships"][0]["brand_id"], "foundry");
    ...
    for resource_type in ["workflow", "event", "artifact"] {
        assert!(tenant_resources
            ...
            .any(|resource| resource["resource_type"] == resource_type
                ...
                && resource["brand_id"] == "foundry"
                && resource["product_id"] == "foundry-core"));
    }

    let tenant_policy_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
    ...
    assert_eq!(
        ...
        "foundry.context.memory_policy.v1"
    );
    assert_eq!(memory_policy["memory_source"], "foundry_memory_router");
    assert_eq!(
    ...
    assert!(context_json["prompt_packet"]["instruction_sources"]
        ...
        .unwrap()
        .iter()
        .any(|source| source == "foundry_operating_context"));
    ...
    assert_eq!(organization_context["brand_id"], "foundry");
    assert_eq!(organization_context["product_id"], "foundry-core");
    ...
    assert_eq!(
        ...
        "foundry.context.personality_decision.v1"
    );
    assert_eq!(
        personality_decision["decision_owner"],
        "foundry_personality_router"
    );
    assert_eq!(
    ...
    assert_eq!(personality_decision["brand_id"], "foundry");
    assert_eq!(personality_decision["product_id"], "foundry-core");
    ...
    assert_eq!(
        ...
        "foundry.context.company_work_decision.v1"
    );
    assert_eq!(
        company_work_decision["decision_owner"],
        "foundry_company_work_router"
    );
    assert_eq!(
        company_work_decision["organization_id"],
        "digital-directive"
    );
    assert_eq!(company_work_decision["brand_id"], "foundry");
    assert_eq!(company_work_decision["product_id"], "foundry-core");
    assert_eq!(
    ...
    assert_eq!(
        mcp_json["result"]["context"]["product"]["id"],
        "foundry-core"
    );

    let mcp_registry_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
    ...
    let mcp_membership_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
...
"#,
    ...
    let read_policy_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
    ...
    let mutate_policy_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
    ...
    let membership_update_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
    ...
    let grant_policy_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
    ...
    foundry()
        ...
        .assert()
        .success();
    let deny_policy_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
    ...
    let expired_policy_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
...
views:
  - id: logistics.route_dashboard
    ...
    surface: ops_console
    type: dashboard
    component: foundry.logistics.route_dashboard
...
"#,
    ...
    let logistics = catalog_json["addons"]
        ...
        .unwrap()
        .iter()
        .find(|addon| addon["id"] == "foundry.addon.logistics")
        .unwrap();
    assert_eq!(
    ...
    let contract_policy_output = foundry()
        .args([
            "addons",
            "contract-policy",
            "--addon-dir",
    ...
    let mcp_contract_policy_input = format!(
        r#"{{"addon_dirs":["{}"],"addon_id":"foundry.addon.logistics","contract_id":"logistics.route_validator"}}"#,
        addon_dir.display()
    );
    let mcp_contract_policy_output = foundry()
        .args([
            "mcp",
            "call",
    ...
    let external_run_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let unsigned_completion_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    .to_string();
    let invalid_complete_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
...
"#,
    ...
    let local_dispatch_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let local_execute_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let local_mcp_dispatch_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    .to_string();
    let local_mcp_execute_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
    ...
    let mcp_dispatch_input = format!(
        r#"{{"addon_dirs":["{}"],"addon_id":"foundry.addon.logistics","contract_id":"logistics.route_validator","input":{{"route_id":"route-002"}},"source":"mcp-test","dry_run":true}}"#,
        addon_dir.display()
    );
    let mcp_dispatch_output = foundry()
        .args([
    ...
    assert_eq!(views_json["schema_version"], "foundry.addon_views.v1");
    ...
    assert_eq!(
        ...
        "foundry.logistics.route_dashboard"
    );
    ...
    let ops_snapshot_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "ops",
    ...
    assert!(ops_snapshot_json["addon_views"]["views"]
        ...
        .unwrap()
        .iter()
        .any(|entry| entry["addon_id"] == "foundry.addon.logistics"
    ...
    assert_eq!(route["source_addon"], "foundry.addon.logistics");
    ...
    assert!(json["workflow_extensions"]
        ...
        .any(|extension| extension["id"] == "route_optimization_workflow"
            && extension["source_addon"] == "foundry.addon.logistics"
    ...
    assert!(json["runtime_contracts"]
        ...
        .any(|contract| contract["id"] == "logistics.route_validator"
            ...
            && contract["source_addon"] == "foundry.addon.logistics"
    ...
    assert!(json["active_addons"]
        .as_array()
        .unwrap()
        .contains(&Value::String("foundry.addon.logistics".to_string())));

    let mut plan_command = foundry();
...
"#,
    ...
    .to_string();
    let mcp_execute_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
...
"#,
    ...
    let vault_secret = "foundry-worker-vault-secret";
    let env_secret = "foundry-worker-env-secret";
    ...
    let execute_output = foundry()
        .env("FOUNDRY_FAKE_VAULT_SECRET", vault_secret)
        .env("FOUNDRY_FAKE_CREDENTIAL_VAULT_ARGS", &vault_args)
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let https_worker_data = serde_json::json!({
        ...
        "auth": "bearer",
        "secret_env": "FOUNDRY_WORKER_HTTPS_TOKEN",
        "allowed_entrypoints": ["gateway.payment.charge"],
        "allowed_contracts": ["gateway.payment_charge"],
        "timeout_seconds": 5,
        "max_response_bytes": 65536,
    ...
    foundry()
        ...
        .assert()
        .success();
    let https_dispatch_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    .to_string();
    let https_output = foundry()
        .env("FOUNDRY_WORKER_HTTPS_TOKEN", env_secret)
        .env("FOUNDRY_EXTERNAL_API_WORKER_HTTPS_MODE", "simulate")
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
...
"#
    ...
    let cli_output = foundry()
        ...
        .clone();
    let cli_json: Value = serde_json::from_slice(&cli_output).unwrap();
    assert_eq!(cli_json["schema_version"], "foundry.event_egress_emit.v1");
    ...
    let mcp_input = serde_json::json!({
        "addon_dirs": [addon_dir.display().to_string()],
        "addon_id": "foundry.addon.partner",
    ...
    assert!(egress_events
        .iter()
        .all(|event| event["observability"]["addon_id"] == "foundry.addon.partner"));
...
"#,
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.tenant_egress
...
"#,
    ...
    let allowed_policy_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "identity",
...
fn builtin_telegram_addon_declares_report_document_egress_for_dry_run_delivery() {
    ...
    let cli_output = foundry()
        .args([
            ...
            "codex",
            "--payload",
            r#"{"workflow_id":"wf_demo","document_ref":"artifacts/final-report.md","caption":"Foundry final report"}"#,
            "--dry-run",
            "--output",
            "json",
        ])
    ...
    let mcp_input = serde_json::json!({
        "addon_id": "foundry.addon.notification",
...
event_adapters:
  - id: partner.signed_webhook_egress
    ...
    auth: hmac
    secret_env: FOUNDRY_TEST_EGRESS_SECRET
...
"#
    ...
    let cli_output = foundry()
        .env("FOUNDRY_TEST_EGRESS_SECRET", secret)
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let mcp_input = serde_json::json!({
        "addon_dirs": [addon_dir.display().to_string()],
        "addon_id": "foundry.addon.signed_partner",
...
fn event_egress_adapter_injects_bearer_secret_without_reporting_value() {
    ...
    let addon_dir = temp.path().join("addons");
    fs::create_dir_all(&addon_dir).unwrap();
    let secret = "foundry-bearer-secret";
    let (endpoint, handle) =
        start_bearer_event_egress_server(2, secret, Some("FOUNDRY_TEST_BEARER_SECRET"));
    fs::write(
        addon_dir.join("bearer-partner.yaml"),
        format!(
            r#"
id: foundry.addon.bearer_partner
...
event_adapters:
  - id: partner.bearer_webhook_egress
    ...
    auth: bearer
    secret_env: FOUNDRY_TEST_BEARER_SECRET
...
"#
    ...
    let cli_output = foundry()
        .env("FOUNDRY_TEST_BEARER_SECRET", secret)
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let mcp_input = serde_json::json!({
        "addon_dirs": [addon_dir.display().to_string()],
        "addon_id": "foundry.addon.bearer_partner",
...
"#,
    ...
    let cli_output = foundry()
        .env("FOUNDRY_FAKE_VAULT_SECRET", secret)
        .env("FOUNDRY_FAKE_CREDENTIAL_VAULT_ARGS", &vault_args)
        .args([
            "--store",
            store.to_str().unwrap(),
            ...
            addon_dir.to_str().unwrap(),
            "--addon",
            "foundry.addon.vault_bearer_partner",
            "--adapter",
    ...
    let mcp_input = serde_json::json!({
        "addon_dirs": [addon_dir.display().to_string()],
        "addon_id": "foundry.addon.vault_bearer_partner",
...
fn event_egress_adapter_supports_https_endpoint_via_simulated_curl_transport() {
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.https_partner
...
event_adapters:
  - id: partner.https_egress
    ...
    permissions: [partner.notify]
    endpoint: "https://partner.example.test/foundry"
...
"#,
    ...
    let cli_output = foundry()
        ...
        .env("FOUNDRY_EVENT_EGRESS_HTTPS_MODE", "simulate")
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    assert_eq!(
        ...
        "https://partner.example.test/foundry"
    );
    assert_eq!(cli_json["delivery"]["auth_scheme"], "bearer");
    ...
    let mcp_input = serde_json::json!({
        "addon_dirs": [addon_dir.display().to_string()],
        "addon_id": "foundry.addon.https_partner",
...
fn event_webhook_ingress_accepts_http_post_and_routes_through_declared_adapter() {
    ...
    fs::write(
        addon_dir.join("logistics.yaml"),
        r#"
id: foundry.addon.logistics_webhook
...
"#,
    ...
    let child = StdCommand::new(assert_cmd::cargo::cargo_bin("foundry"))
        .env("FOUNDRY_ALLOW_INSECURE_LOCAL_WEBHOOK", "true")
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    assert_eq!(report["schema_version"], "foundry.event_webhook_ingress.v1");
    ...
    assert_eq!(
        ...
        "foundry.addon.logistics_webhook"
    );
...
fn event_webhook_ingress_verifies_hmac_before_routing_declared_adapter() {
    ...
    fs::write(
        addon_dir.join("partner.yaml"),
        r#"
id: foundry.addon.partner_signed_webhook
...
"#,
    ...
    let port = reserve_local_port();
    let secret = "foundry-test-webhook-secret-32-bytes-minimum";
    let child = StdCommand::new(assert_cmd::cargo::cargo_bin("foundry"))
        .env("FOUNDRY_TEST_WEBHOOK_SECRET", secret)
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    let bad_response = post_json_with_retry_headers(
        ...
        &[
            ...
            ("X-Foundry-Timestamp", &bad_timestamp),
            ("X-Foundry-Nonce", bad_nonce),
        ],
    );
    assert!(
    ...
    let signed_response = post_json_with_retry_headers(
        ...
        &[
            ...
            ("X-Foundry-Timestamp", &signed_timestamp),
            ("X-Foundry-Nonce", signed_nonce),
        ],
    );
    ...
    assert_eq!(report["auth"]["secret_env"], "FOUNDRY_TEST_WEBHOOK_SECRET");
    assert_eq!(report["auth"]["timestamp_header"], "x-foundry-timestamp");
    assert_eq!(report["auth"]["nonce_header"], "x-foundry-nonce");
...
fn event_service_plan_models_managed_worker_and_webhook_contracts_for_cli_and_mcp() {
    ...
    let webhook_input = serde_json::json!({
        ...
        "route": true,
        "max_requests": 25,
        "hmac_secret_env": "FOUNDRY_PARTNER_WEBHOOK_SECRET",
        "signature_header": "X-Partner-Signature",
        "lease_seconds": 600,
        "heartbeat_seconds": 60
    })
    .to_string();
    let webhook_output = foundry()
        .env("FOUNDRY_ALLOW_INSECURE_LOCAL_WEBHOOK", "true")
        .args([
            "--store",
            store.to_str().unwrap(),
    ...
    assert!(result["command"]
        .as_array()
        .unwrap()
        .contains(&Value::String("FOUNDRY_PARTNER_WEBHOOK_SECRET".to_string())));
...
fn event_service_run_executes_worker_with_persistent_lease_and_mcp_status() {
    ...
    assert_eq!(run_json["schema_version"], "foundry.event_service_run.v1");
...
fn event_services_enforce_project_tenant_policy_for_list_recovery_and_reconcile() {
    ...
    let visible_services_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
    ...
    .to_string();
    let mcp_services_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
    ...
    let hidden_running_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
...
fn event_services_recover_marks_expired_running_leases_stale_for_cli_and_mcp() {
    ...
    let stale_services_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
...
fn event_runtime_reconcile_uses_registry_actions_and_can_execute_supervisor() {
    ...
    assert_eq!(
        ...
        "foundry.event_runtime_registry_snapshot.v1"
    );
    assert_eq!(
...
fn event_runtime_daemon_persists_service_heartbeat_and_stop_file() {
    ...
    assert_eq!(
        ...
        "foundry.event_runtime_daemon_health.v1"
    );
...
fn event_worker_and_service_run_honor_cooperative_stop_file() {
    let temp = tempdir().unwrap();
    let store = temp.path().join("foundry.sqlite");
    let stop_file = temp.path().join("foundry-event-worker.stop");
    fs::write(&stop_file, "stop").unwrap();
    ...
    let webhook_output = foundry()
        .env(
            "FOUNDRY_TEST_STOP_WEBHOOK_SECRET",
            "foundry-test-stop-webhook-secret-32-bytes",
        )
        .args([
            "--store",
            store.to_str().unwrap(),
    ...
    assert_eq!(
        ...
        "foundry.event_webhook_ingress.v1"
    );
    ...
    .to_string();
    let webhook_service_output = foundry()
        .env("FOUNDRY_ALLOW_INSECURE_LOCAL_WEBHOOK", "true")
        .args([
            "--store",
            store.to_str().unwrap(),
    ...
    let stopped_webhook_services_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "events",
...
fn event_service_run_executes_webhook_ingress_with_persistent_status_for_cli_and_mcp() {
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.service_webhook
...
"#,
    ...
    let cli_child = StdCommand::new(assert_cmd::cargo::cargo_bin("foundry"))
        .env("FOUNDRY_ALLOW_INSECURE_LOCAL_WEBHOOK", "true")
        .args([
            "--store",
            store.to_str().unwrap(),
    ...
    assert_eq!(cli_json["schema_version"], "foundry.event_service_run.v1");
    ...
    .to_string();
    let mcp_child = StdCommand::new(assert_cmd::cargo::cargo_bin("foundry"))
        .env("FOUNDRY_ALLOW_INSECURE_LOCAL_WEBHOOK", "true")
        .args([
            "--store",
            store.to_str().unwrap(),
    ...
    let services_output = foundry()
        ...
        .clone();
    let services_json: Value = serde_json::from_slice(&services_output).unwrap();
    assert_eq!(services_json["schema_version"], "foundry.event_services.v1");
    assert_eq!(services_json["service_count"], 2);
    assert!(services_json["services"]
        .as_array()
        .unwrap()
...
#[test]
fn addon_runtime_dispatch_runs_safe_foundry_core_builtin_contracts() {
    let temp = tempdir().unwrap();
    let store = temp.path().join("foundry.sqlite");
    let addon_dir = temp.path().join("addons");
    fs::create_dir_all(&addon_dir).unwrap();
...
fn addon_runtime_dispatch_worker_processes_queued_dispatches_for_cli_and_mcp() {
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.builtin_worker
...
"#,
    ...
    let mcp_output = foundry()
        ...
        .arg("foundry.addons.dispatch_worker")
        .arg("--input")
...
fn multimodal_addon_runtime_dispatch_uses_guarded_builtin_benchmark() {
    ...
    assert_eq!(output["kind"], "foundry_multimodal_runtime_benchmark");
    assert_eq!(
...
fn addon_validation_reports_missing_dependencies_without_planning() {
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.broken
name: Broken Addon
version: 0.1.0
dependencies:
  - id: foundry.addon.missing
    required: true
capabilities:
...
event_adapters:
  - id: broken.webhook
    ...
    permissions:
      - broken.missing_permission
compatibility:
  foundry_version_req: ">=999.0.0"
  api_versions:
    - foundry.addon_manifest.v999
  runtimes:
    - wasm
    - quantum
  features:
...
"#,
    ...
    let json: Value = serde_json::from_slice(&output).unwrap();
    assert_eq!(json["schema_version"], "foundry.addon_validation.v1");
...
fn addon_validation_reports_invalid_event_wiring_references() {
    ...
    fs::write(
        ...
        r#"
id: foundry.addon.broken_events
...
"#,
    ...
    assert!(denied_stderr.contains("foundry.addon.payments:payments.charge"));

    let denied_contracts = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let denied_contract_policy = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let denied_dispatch_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let approved_contracts = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let approved_contract_policy = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let queued_payment_dispatch = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let revoked_run_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let permissions_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
            ...
            "foundry.addons.permissions",
            "--input",
            r#"{"addon_id":"foundry.addon.payments","status":"approved"}"#,
            "--output",
            "json",
        ])
        .assert()
    ...
    let enabled_capabilities = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    foundry()
        .args([
            ...
            "foundry.addons.revoke_permission",
            "--input",
            r#"{"addon_id":"foundry.addon.payments","permission_id":"payments.charge","approved_by":"arthur","source":"test"}"#,
            "--output",
            "json",
        ])
        .assert()
    ...
    let unauthorized_capabilities = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let resolve_after_revoke = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    assert_eq!(blocked_payment["source_addon"], "foundry.addon.payments");
...
"#,
    ...
    let package_output = foundry()
        .args([
            ...
            manifest.to_str().unwrap(),
            "--repository",
            "https://example.com/foundry/addons/fiscal.git",
            "--channel",
            "beta",
            "--signature",
            "sig-demo",
    ...
    assert_eq!(package_json["schema_version"], "foundry.addon_package.v1");
    assert_eq!(package_json["status"], "addon_package_ready");
    assert_eq!(package_json["package_id"], "foundry.addon.fiscal@0.1.0");
    ...
    let package_mcp_input = format!(
        r#"{{"manifest":"{}","repository":"registry://foundry/fiscal","channel":"stable"}}"#,
        manifest_v2.to_string_lossy().replace('\\', "\\\\")
    );
    let package_mcp_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
    ...
    assert_eq!(install_json["schema_version"], "foundry.addon_lifecycle.v1");
    assert_eq!(install_json["status"], "installed");
    assert_eq!(install_json["addon"]["id"], "foundry.addon.fiscal");
    assert_eq!(
    ...
    let capabilities_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    assert_eq!(upgrade_json["schema_version"], "foundry.addon_lifecycle.v1");
    ...
    let upgraded_capabilities_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let downgraded_capabilities_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let disabled_capabilities_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
            ...
            "foundry.addons.capabilities",
            "--input",
            r#"{"addon_id":"foundry.addon.fiscal","lifecycle":"disabled"}"#,
            "--output",
            "json",
        ])
        .assert()
    ...
    let capabilities_empty = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
...
"#,
    ...
    let install_plan_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let empty_installed_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let upgrade_plan_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let capabilities_after_preview = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let disable_plan_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
            ...
            "foundry.addons.lifecycle_plan",
            "--input",
            r#"{"action":"disable","id":"foundry.addon.support"}"#,
            "--output",
            "json",
        ])
        .assert()
    ...
    let installed_after_disable_preview = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
...
"#,
    ...
    let repository = "registry://foundry/fiscal";
    let channel = "stable";
    let package_id = "foundry.addon.fiscal@1.2.0";
...
fn addon_sync_registry_fetches_indexed_packages_into_marketplace_cache() {
    ...
    fs::write(
        &manifest,
        r#"
id: foundry.addon.notifications.email
...
"#,
    ...
    let repository = "registry://foundry/notifications";
    let channel = "stable";
    let package_id = "foundry.addon.notifications.email@0.3.0";
    let signing_key = SigningKey::from_bytes(&[29u8; 32]);
    let public_key = test_hex_encode(signing_key.verifying_key().as_bytes());
    let payload = addon_package_payload_bytes(
        &manifest,
        package_id,
        "foundry.addon.notifications.email",
        "0.3.0",
        repository,
        channel,
    );
    ...
    assert_eq!(lock_json["schema_version"], "foundry.addon_package_lock.v1");
    ...
    let locked_sync_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
...
"#,
    ...
    let repository = "registry://foundry/payments";
    let channel = "stable";
    let package_id = "foundry.addon.payments@1.0.0";
    ...
    assert_eq!(trust_json["schema_version"], "foundry.addon_trust_store.v1");
    assert_eq!(trust_json["key"]["status"], "trusted");
    assert_eq!(trust_json["key"]["public_key"], public_key);

    let publish_allowed = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "addons",
    ...
    let marketplace_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
            ...
            "foundry.addons.marketplace",
            "--input",
            r#"{"repository":"registry://foundry/payments","channel":"stable"}"#,
            "--output",
            "json",
        ])
        .assert()
    ...
    assert!(install_json["result"]["lifecycle"]["addon"]["source"]
        ...
        .unwrap()
        .starts_with(
            "marketplace:registry://foundry/payments:stable:foundry.addon.payments@1.0.0#"
        ),);
...
"#,
    ...
    let workflow_list_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "list",
    ...
    let explicit_workflow_output = foundry()
        .args([
            "--store",
            store.to_str().unwrap(),
            "mcp",
...
fn plan_records_capability_context_and_event_policy_in_intent() {
    ...
    assert_eq!(intent["schema_version"], "foundry.intent.v2");
    assert_eq!(
    ...
    assert!(intent["capability_resolution"]["runtime_contracts"]
        ...
        .any(
            |contract| contract["id"] == "creative_workspace.planning_strategy"
                && contract["contract_type"] == "planning_strategy"
                && contract["source_addon"] == "foundry.addon.visual_workspace"
        ));
    assert_eq!(
