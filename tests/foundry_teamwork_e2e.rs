...
use foundry_core::teamwork::{prepare_teamwork_worktrees, TeamworkWorktreePrepareOptions};
...
/// Helper function to locate and execute the foundry CLI binary
fn foundry() -> Command {
...
fn save_ready_executor(store: &FoundryStore, executor: &str) {
...
fn prepare_teamwork_execution_fixture(temporary_root: &Path, store_path: &Path, workflow_id: &str) {
    ...
    prepare_teamwork_worktrees(
        ...
        TeamworkWorktreePrepareOptions {
            ...
            repository,
            worktree_root,
            branch_prefix: "foundry/teamwork-e2e".to_string(),
...
fn next_request_executor(store_path: &Path, run_id: &str) -> String {
    let store = FoundryStore::open(store_path).unwrap();
    let run = foundry_core::request::load_run_record(&store, run_id).unwrap();
    let workflow = store.load_workflow(&run.workflow_id).unwrap();
    let completed = workflow
        .tasks
        .iter()
        .filter(|task| task.status == foundry_core::graph::TaskStatus::Completed)
        .map(|task| task.id.as_str())
        .collect::<BTreeSet<_>>();
    let Some(task) = workflow.tasks.iter().find(|task| {
        task.status == foundry_core::graph::TaskStatus::Pending
            && task
                .dependencies
                .iter()
                .all(|dependency| completed.contains(dependency.as_str()))
    }) else {
        return "foundry_cli".to_string();
    };
...
fn test_f1_cli_detached_json_format() {
    ...
    assert_eq!(json["schema_version"], "foundry.teamwork.plan.v1");
...
fn test_f1_cli_detached_human_format() {
    ...
    assert!(stdout_str.contains("FOUNDRY TEAMWORK EXECUTION PLAN"));
...
/// Test 8: Mock HTTP server endpoint query via FOUNDRY_BENCHMARK_URL redirection
#[test]
fn test_f2_mock_benchmark_url_fetch() {
    ...
    let output = foundry()
        ...
        .env("FOUNDRY_BENCHMARK_URL", &mock_server.url)
        .assert()
        .success()
        .get_output()
        .stdout
...
fn test_f4_persistence_workflows_runs_schema() {
    ...
    let _store = FoundryStore::open(&store_path).unwrap();
    let conn = Connection::open(&store_path).unwrap();
...
fn test_f4_persistence_cached_benchmark_rankings() {
    ...
    let intent_failed = foundry_core::intent::parse_intent("Plan with failures");
    let mut wf_failed = foundry_core::graph::create_workflow(intent_failed);
    wf_failed.id = "wf_failed".to_string();
    wf_failed.status = "failed".to_string();
    if let Some(task) = wf_failed.tasks.first_mut() {
        task.status = foundry_core::graph::TaskStatus::Failed;
    }
    let wf_failed_json = serde_json::to_string(&wf_failed).unwrap();

    let intent_completed = foundry_core::intent::parse_intent("Completed plan");
    let mut wf_completed = foundry_core::graph::create_workflow(intent_completed);
...
fn test_t2_f2_benchmark_server_500() {
    ...
    let _assert = foundry()
        ...
        .env("FOUNDRY_BENCHMARK_URL", &server.url)
        .assert()
        .success();
}
...
fn test_t2_f2_benchmark_server_unreachable() {
    ...
    let _assert = foundry()
        ...
        .env("FOUNDRY_BENCHMARK_URL", unreachable_url)
        .assert()
        .success();
}
...
fn test_t2_f3_step_cancelled_workflow() {
    ...
    let intent = foundry_core::intent::parse_intent("Goal");
    let mut workflow = foundry_core::graph::create_workflow(intent);
    workflow.status = "cancelled".to_string();
    store.save_workflow(&workflow).unwrap();

    let run = foundry_core::request::create_run_record(&workflow, "foundry_cli", "cancelled");
...
fn test_t2_f3_step_failed_workflow() {
    ...
    let run = foundry_core::request::create_run_record(&workflow, "foundry_cli", "failed");
    let run_id = run.run_id.clone();
    let run_val = serde_json::to_value(&run).unwrap();
...
fn test_t2_f3_lease_already_leased() {
    ...
    let intent = foundry_core::intent::parse_intent("Goal");
    let workflow = foundry_core::graph::create_workflow(intent);
    store.save_workflow(&workflow).unwrap();
...
fn test_t2_f3_simulation_no_tasks() {
    ...
    let intent = foundry_core::intent::parse_intent("Empty Goal");
    let mut workflow = foundry_core::graph::create_workflow(intent);
...
fn test_t2_f3_complete_task_extreme_values() {
    ...
    let run = foundry_core::request::create_run_record(&workflow, "foundry_cli", "accepted");
    let run_val = serde_json::to_value(&run).unwrap();
...
fn test_t3_runtime_stepping_updates_lineage_status() {
    ...
    let intent = foundry_core::intent::parse_intent("Stepping test goal");
    let workflow = foundry_core::graph::create_workflow(intent);
    store.save_workflow(&workflow).unwrap();
...
fn test_t3_heuristics_execution_cost_ledger_updates() {
    ...
    let intent = foundry_core::intent::parse_intent("Cost test goal");
    let workflow = foundry_core::graph::create_workflow(intent);
    store.save_workflow(&workflow).unwrap();
...
fn test_t3_lease_expiration_forces_failure_and_improve_candidates() {
    ...
    let intent = foundry_core::intent::parse_intent("Lease expiration goal");
    let mut workflow = foundry_core::graph::create_workflow(intent);
    workflow.status = "running".to_string();
    store.save_workflow(&workflow).unwrap();

    let mut run = foundry_core::request::create_run_record(&workflow, "foundry_cli", "running");
