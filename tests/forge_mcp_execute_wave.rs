...
fn mcp_manifest_exposes_bounded_executor_wave_without_auto_completion() {
    let tool = mcp_tools_manifest()
        .tools
        .into_iter()
        .find(|tool| tool.name == "forge.run.execute_wave")
        .expect("executor wave must be exposed over MCP");

    assert!(tool.async_safe);
    assert!(tool.mutates_workflow);
    assert_eq!(tool.output_schema, "forge.request_executor_wave.v1");
    assert!(tool
        .description
        .contains("Process receipts never complete or promote Forge tasks"));
    assert!(tool
        .forge_command
        .windows(2)
