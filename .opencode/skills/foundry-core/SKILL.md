...
Install the complete router-plus-modules bundle:

```bash
foundry skill install --target codex --target opencode --home "$HOME" --output json
```

The installer writes this router and every domain skill beside it for each selected target, plus the shared `.agents/skills` bundle. In this repository, canonical domain sources live at `.agents/skills/<skill-name>/SKILL.md`.

After installation or changes in executor/runtime availability, run:

```bash
foundry sync all --home "$HOME" --output json
```

Detection does not authorize execution. Add `--allow <executor>` only after explicit human approval, and do not dispatch to an executor until local policy marks it allowed.

For asynchronous skill adoption, start a Foundry-owned request and return its `run_id`:

```bash
foundry request start --goal "<objective>" --origin codex|opencode|agy|claude|skill --output json
```
