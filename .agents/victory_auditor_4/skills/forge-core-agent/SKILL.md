...
- **Local executor policy**: Mark local CLI tools (e.g. `antigravity`) as allowed or disallowed.
...
forge executor policy --allow "antigravity" --output json
```
...
```bash
forge executor credentials set --adapter "google" --key "GEMINI_API_KEY" --value "secret_value" --output json
```

### Quotas and Limits (`ai-limits`)
Forge monitors usage metrics (tokens, costs, execution duration) against the configured policies.
```bash
forge executor limits --profile "gemini-flash-coder" --token-limit 500000 --cost-limit-usd 2.50 --output json
```
