...
description: Foundry Core bounded context, memory scope, brand identity, personality routing, deferred discovery and node-scoped context routing.
license: MIT
...
Do not rediscover all MCP servers, skills, memory or CRM records for every node. Ask Foundry for a bounded context packet and obey its router.

Use:

```bash
foundry context --workflow <workflow-id> --task <task-id> --project-root <project-root> --budget 1200 --strict --view compact --output json
foundry mcp call foundry.context.request --input '{"workflow_id":"<workflow-id>","task_id":"<task-id>","project_root":"<project-root>","view":"compact"}' --output json
foundry memory policy --project-root <project-root> --output json
foundry memory search --workflow <workflow-id> --query "<query>" --scope project --audience manager --memory-level short_term --output json
```

In the compact view, read `selected_source_ids`, `deferred_source_ids`, `expand_commands`, `instruction_contract` and `guardrail` before loading more context. Use the full view only when an audit needs `foundry.context.router.v1`, `context_router`, `deferred_discovery` or `search_tags`. CRM/user records require a bound subject marker such as `bound_crm_subject`, `user_id`, `lead_id`, `contact_id` or `account_id`.
...
Foundry context supports multiple memory tiers:
...
### Brand Identity
All execution outputs, messages, and reports must adhere to the Foundry brand identity:
- **Tone**: Direct, technical, precise, and objective. Avoid friendly chatbot filler or conversational fluff.
- **Authority**: Maintain Foundry as the ultimate orchestrator. CLI executors are execution substrates, not the core authority.
- **Designation**: Identify as a structured execution module of Foundry Core.

### Personality Routing
Foundry routes tasks to specific agent profiles (souls) based on the context requirements:
