...
```bash
forge request start --goal "<objective>" --origin codex \
...
forge request execute-wave --run <run-id> --executor auto --context-budget 4096 \
...
Explicit request lanes use the same Core materializer as `forge plan` and MCP
`forge.run.start`, and persist under `core_orchestration.parallel_team`.
...
an admission ceiling: Forge dispatches dependency-ready tasks, and every
...
`forge request complete-task --evidence-command "<passing gate>"
--evidence-exit-code <observed-exit-code>`. The exit code must be the value
actually observed by the caller; Forge never fabricates `0`. Forge returns
`handoff_required` while execution evidence is absent.

`forge request execute-wave` is the bounded real-process bridge for an admitted
