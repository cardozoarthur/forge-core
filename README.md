...
5. `forge interactive architecture` mostra a bússola entre goal1/goal2/goal3, evidências atuais, gaps, plano incremental, dependências, reuso e benchmarks.
6. `forge smoke operational-tui --output json` prova localmente que a TUI abre, cria um fluxo demonstrável e expõe workflows, eventos/schedules, Addons/capabilities, custos e handoffs/approvals.
7. `forge smoke forge-first-harness --output json` prova localmente que headroom, adoption-plan, bootstrap dry-run, shim Forge-owned e exec dry-run funcionam sem executar ou modificar a CLI externa.
8. `forge smoke replacement-cli-evidence --output json` prova que o gate de CLI substituta coleta as evidências prontas, reporta os skips que dependem de manifesto aprovado e não promove o milestone automaticamente.
...
- interactive home dashboard through `forge interactive home --project-root <project-root>` and MCP `forge.interactive.home` with `project_root`, including `workflow_sidebar_panel`, `replacement_cli_panel`, `multimodal_runtime_panel`, `harness_mode_panel`, harness headroom stats, executor compatibility, operational cockpit modifier-lane review, read-only `event_runtime_panel`, project-root-aware release gates and `/harness` quick action so TUI/web/agent dashboards can see grouped workflow navigation, replacement-grade CLI readiness, Addon-owned multimodal runtime readiness, effective Forge-first mode, source, project config status, token-headroom savings, CLI adapter readiness, pending inbound event work and pending strategic runtime mutations before opening brain shells
