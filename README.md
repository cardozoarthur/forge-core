...
- harness utilities for token-headroom analysis, Forge-first CLI wrapper planning, non-destructive PATH shim installation and reversible stdout/stderr headroom receipts across Codex, Claude, Gemini and OpenCode-style executors
...
forge harness install-shims --shim-dir .forge/bin --executor codex --real-cmd /usr/local/bin/codex --forge-first --workflow <workflow-id> --run <run-id> --context-budget 8000 --output json
...
forge mcp call forge.harness.install_shims --input '{"shim_dir":".forge/bin","executor":"codex","real_cmd":"/usr/local/bin/codex","forge_first":true,"workflow_id":"<workflow-id>","run_id":"<run-id>","context_budget":8000}' --output json
