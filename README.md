...
forge harness token-headroom --content "$(cat logs/build.log)" --kind log --budget-tokens 1200 --source build-log --output json
...
forge mcp call forge.harness.token_headroom --input '{"content":"error: failed\nwarning: retry\nfinal status ok","content_kind_hint":"log","budget_tokens":120,"source":"demo-log","reversible":true}' --output json
...
`forge harness token-headroom --content <payload> --kind log --budget-tokens <n> --output json` and MCP `forge.harness.token_headroom` provide deterministic local-first compression reports for logs, search results, JSON, code and text, including original/compressed hashes, estimated token savings, budget status and a retrieval ref. `forge harness wrap-plan --executor codex|claude|gemini|opencode --cmd <arg> --forge-first --output json` and MCP `forge.harness.wrap_plan` produce a Forge-first CLI environment plan, preserving executor-specific needs such as Claude tool search while keeping actual process wrapping as an explicit guarded follow-up.
