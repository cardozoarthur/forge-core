...
- harness utilities for token-headroom analysis, Forge-first CLI wrapper planning, non-destructive PATH shim installation, shim status auditing and reversible stdout/stderr headroom receipts across Codex, Claude, Gemini and OpenCode-style executors
...
forge harness install-shims --shim-dir .forge/bin --executor codex --forge-first --workflow <workflow-id> --run <run-id> --context-budget 8000 --output json
...
forge shells --executor codex --output json
...
forge mcp call forge.harness.install_shims --input '{"shim_dir":".forge/bin","executor":"codex","forge_first":true,"workflow_id":"<workflow-id>","run_id":"<run-id>","context_budget":8000}' --output json
forge mcp call forge.harness.shim_status --input '{"shim_dir":".forge/bin","executor":"codex"}' --output json
forge mcp call forge.shell.launch_plan --input '{"executor":"codex"}' --output json
