...
- interactive home dashboard through `forge interactive home --project-root <project-root>` and MCP `forge.interactive.home` with `project_root`, including `workflow_sidebar_panel`, `harness_mode_panel`, harness headroom stats, executor compatibility, operational cockpit modifier-lane review, read-only `event_runtime_panel`, project-root-aware release gates and `/harness` quick action so TUI/web/agent dashboards can see grouped workflow navigation, effective Forge-first mode, source, project config status, token-headroom savings, CLI adapter readiness, pending inbound event work and pending strategic runtime mutations before opening brain shells
