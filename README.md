...
`forge improve apply-event-policy` can convert a selected `prefer_deterministic_node` recommendation into an approved workflow revision with rollback metadata and an equivalence gate; it still does not promote the change without benchmark and validation evidence.
