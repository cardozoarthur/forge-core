...
`forge context` emits a versioned context packet (`forge.context.v18`) with a deterministic
`task_local_revisioned_persona_compressed_executor_policy_subflow_checkpoint_dependencies_handoff_budget_summary_required_first_content_addressed_shards_budget_ledger_quality_contract_v18` routing policy.
...
profile metadata, a versioned routing contract, execution policy metadata, dependency readiness summaries, proposed
...
`handoff_blockers`, a `routing_summary`, a versioned `routing_contract`, a versioned `routing_quality` score/warning contract and a versioned `routing_fingerprint`
