...
- derived event observability index over the global event timeline, grouped by tenant, workflow, node and Addon with duration, retry and wait aggregates
...
`forge events observability --organization <organization-id> --node <node-ref> --addon <addon-id> --output json` returns `forge.event_observability_index.v1`, a derived normalized index over the global timeline. It groups event evidence by tenant, workflow, node and Addon, aggregates duration, retry and wait totals, keeps severity/category counts and returns a cursor-compatible page of matching envelopes. This is the query surface for operators, dashboards and improvement loops; a physically materialized observability table remains future work. The same report is available through `forge.events.observability`.
