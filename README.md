...
- replacement-grade CLI readiness through `forge interactive replacement-cli --project-root <project-root>` and MCP `forge.interactive.replacement_cli` with `project_root`, using `forge.interactive.replacement_cli.v1` to aggregate operator home, workflow operations, patch editing UX, action discovery, project-scoped harness/session controls, observability, approvals, milestone evidence and provider-level `provider_readiness` rows without launching child CLIs or mutating state
