...
- replacement-grade CLI readiness through `forge interactive replacement-cli --project-root <project-root>` and MCP `forge.interactive.replacement_cli` with `project_root`, using `forge.interactive.replacement_cli.v1` to aggregate operator home, workflow operations, patch editing UX, action discovery, project-scoped harness/session controls, observability, approvals, milestone evidence, provider-level `provider_readiness` rows, plan-only `provider_wrapper_plans` and static `provider_wrapper_manifest_audit` for approved connected-brain wrappers without launching child CLIs, executing models, mutating project files or counting preparation as release evidence; the audit checks the project manifest, selected provider, approvals, model declaration, safety flags and wrapper executable metadata before any collection command runs
...
forge milestone prepare-evidence-inputs --version 0.5 --capability replacement_grade_cli --project-root . --connected-brain <provider-id> --provider-command <absolute-wrapper-path> --model-id <approved-model-id> --approval-ref <approval-ref> --apply --approved-by arthur --output json
...
`forge harness wrap-plan` also returns `connected_brain_provider_wrapper` (`forge.harness.connected_brain_provider_wrapper.v1`), a read-only bridge that points `.forge/connected-brain-runtimes.json` provider templates at the absolute Forge-owned shim path `.forge/bin/<executor>`, exposes the exact `install-shims`, `prepare-evidence-inputs`, `evidence-plan` and `collect-evidence` commands, and still reports `counts_as_release_evidence=false` until an approved manifest plus separate provider execution receipt exists.
