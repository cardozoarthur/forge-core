...
`forge improve apply-event-policy` can convert selected event-policy recommendations into approved workflow revisions with rollback metadata and an equivalence gate. It covers deterministic node conversion, validation/rework gates, tighter context routing and supervised waits; it still does not promote a change without benchmark and validation evidence.
