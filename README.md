...
`forge cost incremental --after-sequence <global-event-id> --output json` returns `forge.cost_ledger_incremental.v1`, scanning `global_events` after a cursor, deduplicating affected workflow ids and materializing only those workflows into the normalized cost index. It returns `next_after_sequence` for the next run and avoids recording a self-feeding event. The same operation is available through `forge.cost.incremental`.
