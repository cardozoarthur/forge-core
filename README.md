...
- outbound Addon event adapter execution for declared `http`/`webhook` egresso, with direction/action/event-type/permission checks, endpoint allowlist, env-backed or credential-vault-backed HMAC/Bearer secrets, `http://` posting, `https://` posting through controlled curl, typed JSON envelope, timeout, response hash report and persisted global timeline audit
...
- harness utilities for token-headroom analysis and Forge-first CLI wrapper planning across Codex, Claude, Gemini and OpenCode-style executors
...
- opt-in `tenant_policy_mode: enforce` in operating context, blocking `plan`, async `request start`, inbound `start_workflow`, context handoff, run drive/step, workflow mutations, task leases, checkpoints, interactions, schedule execution and patch/artifact mutations when the active user lacks membership or the membership role lacks the required permission
- tenant-aware workflow event stream projection through CLI and MCP
...
forge harness wrap-plan --executor codex --cmd codex --cmd --dangerously-bypass-approvals-and-sandbox --forge-first --workflow <workflow-id> --run <run-id> --context-budget 8000 --output json
forge harness exec --executor codex --forge-first --workflow <workflow-id> --run <run-id> --context-budget 8000 --output json -- codex --version
...
forge cost materialize --workflow <workflow-id> --output json
...
forge events services --kind worker --status completed --output json
...
forge mcp call forge.events.timeline --input '{"organization_id":"digital-directive","limit":50}' --output json
...
forge mcp call forge.harness.wrap_plan --input '{"executor":"claude-code","command":["claude","--model","sonnet"],"forge_first":true,"workflow_id":"<workflow-id>","run_id":"<run-id>","context_budget":8000,"token_headroom":true}' --output json
forge mcp call forge.harness.exec --input '{"executor":"codex","command":["codex","--version"],"forge_first":true,"workflow_id":"<workflow-id>","run_id":"<run-id>","dry_run":true}' --output json
forge mcp call forge.cost.maintain --input '{"workflow_id":"<workflow-id>","bucket":"day","group_by":"source_kind","retention_days":31}' --output json
...
forge mcp call forge.cost.materialize --input '{"workflow_id":"<workflow-id>","source_kind":"observed_event"}' --output json
...
`forge cost ledger --workflow <workflow-id> --output json` returns `forge.cost_ledger.v1`, a read-only cost ledger grouped by workflow, node, tenant and detected Addon source. It combines planned task cost estimates with observed executor/event costs and token counts from workflow events, so operators can inspect cost by organization/brand/product and decide where deterministic nodes or Addon policies should replace repeated AI work. The same report is available through `forge.cost.ledger`.
`forge cost materialize --workflow <workflow-id> --output json` returns `forge.cost_ledger_index.v1`, writing a normalized SQLite index with one row per planned task cost and one row per observed event cost. The materialized rows carry workflow, tenant, Addon, executor, model-call flags, estimated cost, observed cost and token totals, so dashboards and improvement policies can query cost history without reparsing whole workflow JSON. The same operation is available through `forge.cost.materialize`.
`forge cost history --bucket day --group-by source_kind --output json` returns `forge.cost_ledger_history.v1`, a read-only hour/day rollup over the materialized `cost_ledger_index`. It can group buckets by none, tenant, workflow, source kind, Addon or executor and reuses the normalized planned/observed row summary for dashboards and Cost OS policy loops. The same report is available through `forge.cost.history`.
`forge cost maintain --workflow <workflow-id> --bucket day --group-by source_kind --retention-days 31 --output json` returns `forge.cost_ledger_maintenance.v1`, an idempotent maintenance receipt that materializes the normalized index and immediately returns the requested rollup plus a plan-only retention policy. It is safe for runtime/schedule workers to call periodically; physical pruning remains blocked until an approval-gated retention surface exists. The same operation is available through `forge.cost.maintain`.
...
`forge improve apply-event-policy` can convert selected event-policy recommendations into approved workflow revisions with rollback metadata and an equivalence gate. `forge improve benchmark-event-policy` then validates the latest applied policy against the current workflow, rollback readiness and workflow validation, producing promotion evidence without changing revisions or auto-promoting. The flow covers deterministic node conversion, validation/rework gates, tighter context routing and supervised waits; promotion remains human-approved and benchmark-gated.
