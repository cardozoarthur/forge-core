...
- CLIs such as Codex, `agy` (Antigravity) and OpenCode can call Forge directly for simpler adoption.
- Gemini CLI is treated as a legacy executor path; new Forge routing should prefer Codex or `agy` unless a local policy explicitly keeps Gemini for old workflows.
...
4. `forge interactive guided-cockpit --output json` expõe a mesma experiência em contrato scriptável/MCP.
5. `forge interactive task-board` mostra tarefas, handoffs, aprovações e checkpoints.
6. `forge interactive operating-context` mostra tenant, memória, personalidade, contexto de marca/design, gates de prompt-packet e checklist de empresa antes do handoff.
7. `forge interactive workflow-mutation` junta DAG, task-board, modifier lane, handoffs, custos, logs e comandos seguros para replanejar goals/nodes/brains/artifacts sem aplicar mutações por atalho.
8. `forge interactive event-runtime` mostra inbox, workers e o ciclo `start_workflow`, `continue_workflow`, `pause_workflow`, `resume_workflow`, `modify_workflow` e `end_workflow`.
9. `forge interactive improvement-loop` mostra candidatos de auto-melhoria com logs, custos, validação, qualidade de contexto, outcome e comandos governados antes de qualquer mutação.
10. `forge interactive architecture --project-root .` mostra a bússola entre goal1/goal2/goal3 com contexto operacional do projeto, evidências atuais, gaps, plano incremental, dependências, reuso e benchmarks.
...
12. `forge smoke forge-first-harness --output json` prova localmente que headroom, adoption-plan, bootstrap dry-run, shim Forge-owned, ativação one-shot de PATH e exec dry-run funcionam sem executar ou modificar a CLI externa.
...
- advanced guided cockpit opened by typing only `forge`, with a terminal frame for workflow, execution, realtime and safe-action panes plus `j`/`k` focus, `enter` open and `r` refresh; `forge interactive guided-cockpit` and MCP `forge.interactive.guided_cockpit` expose `forge.interactive.guided_cockpit.v1`: four panes, eight end-to-end operator steps, current-step detection, preview commands, confirmation gates, rollback hints and safe-action policy without mutating state
- interactive home dashboard through `forge interactive home --project-root <project-root>` and MCP `forge.interactive.home` with `project_root`, including `workflow_sidebar_panel`, `workflow_mutation_panel`, `improvement_loop_panel`, `replacement_cli_panel`, `multimodal_runtime_panel`, `harness_mode_panel`, harness headroom stats, executor compatibility, operational cockpit modifier-lane review, read-only `event_runtime_panel`, project-root-aware release gates and `/harness` quick action so TUI/web/agent dashboards can see grouped workflow navigation, workflow mutation/replanning evidence, self-improvement candidates from logs/costs/validation/outcome evidence, replacement-grade CLI readiness, Addon-owned multimodal runtime readiness, effective Forge-first mode, source, project config status, token-headroom savings, CLI adapter readiness, pending inbound event work and pending strategic runtime mutations before opening brain shells
- dedicated UI composition through `forge interactive ui-composition --project-root <project-root> --output json` and MCP `forge.interactive.ui_composition`, exposing `forge.interactive.ui_composition.v1` with ordered layout regions, Core widgets, Addon widgets, renderer families and safe commands for TUI, web and agent dashboards without loading the full home dashboard
- architecture compass through `forge interactive architecture --project-root <project-root>` and MCP `forge.interactive.architecture`, exposing the goal1/goal2/goal3 source document hashes, project operating context, architecture tracks, implementation evidence, gaps, execution plan increments, acceptance gates, dependencies, reuse opportunities, benchmark boundaries and next commands without mutating state
- Core boundary audit through `forge interactive core-boundary` and MCP `forge.interactive.core_boundary`, exposing `forge.interactive.core_boundary.v1` to prove the Core kernel only owns universal responsibilities, domain capabilities stay Addon-owned, compatibility executors remain visible, and goal3 Addon gates have concrete evidence commands
- replacement-grade CLI readiness through `forge interactive replacement-cli --project-root <project-root>` and MCP `forge.interactive.replacement_cli` with `project_root`, using `forge.interactive.replacement_cli.v1` to aggregate operator home, workflow operations, patch editing UX, action discovery, project-scoped harness/session controls, observability, approvals, milestone evidence, provider-level `provider_readiness` rows, plan-only `provider_wrapper_plans` and static `provider_wrapper_manifest_audit` for approved connected-brain provider adapters without launching child CLIs, executing models, mutating project files or counting preparation as release evidence; the audit checks the project manifest, selected provider, approvals, model declaration, safety flags and adapter executable metadata before any collection command runs
- Addon-owned multimodal runtime readiness through `forge interactive multimodal-runtime --project-root <project-root>` and MCP `forge.interactive.multimodal_runtime`, using `forge.interactive.multimodal_runtime.v1` to expose the `forge.addon.multimodal` capability boundary, feature flag, guard state, safe install/readiness/benchmark/demo templates, project-scoped Addon commands and `production_runtime_evidence_plan` blockers without installing models, executing models, accessing devices or mutating workflows
...
- interactive workflow mutation planner through `forge interactive workflow-mutation` and MCP `forge.interactive.workflow_mutation`, using `forge.interactive.workflow_mutation.v1` to combine DAG, task-board, modifier lane, handoffs, costs, events, pending proposals and safe `workflow update-goal`, `workflow update-node-brain` and `workflow attach-artifact` commands without applying mutations from the read-only panel
- interactive improvement loop through `forge interactive improvement-loop` and MCP `forge.interactive.improvement_loop`, using `forge.interactive.improvement_loop.v1` to expose ranked self-improvement candidates, critical/high counts, parallel-ready handoffs, avoidable AI cost, final-outcome gaps, stale/attention signals, structured-log counts, cost totals, validation failures and governed next commands without mutating state
...
forge harness activation-profile --shim-dir .forge/bin --executor codex --project-root . --context-budget 8000 --token-headroom --output json
...
forge milestone prepare-evidence-inputs --version 0.5 --capability replacement_grade_cli --project-root . --connected-brain <provider-id> --provider-command <absolute-provider-adapter-path> --model-id <approved-model-id> --approval-ref <approval-ref> --apply --approved-by arthur --output json
...
forge milestone collect-ready-evidence --version 0.5 --project-root . --approved-by arthur --origin codex --output json
...
forge mcp call forge.harness.headroom_stats --input '{"source":"demo-log","limit":5}' --output json
...
`forge harness shim-status` includes `activation_diagnostic` (`forge.harness.shim_activation_diagnostic.v1`) so CLI, MCP and TUI consumers can distinguish active, recommended, blocked and unavailable Forge-first activation states, including one-shot PATH activation, activation-profile and verification commands without editing shell files.

`forge interactive action-registry --query workflow` now projects workflow-mutation actions for recent workflows, including `workflow.update_goal.<workflow-id>`, `workflow.update_node_brain.<workflow-id>` and `workflow.attach_artifact.<workflow-id>`. These entries come from the universal workflow mutation surface, require approval, stay preview-only through `forge interactive action-invocation`, and keep mutations routed through Forge workflow APIs instead of embedding domain-specific UI logic.
...
`forge request step` and `forge.run.step` expose the next supervised routing boundary but never count metadata as task execution. Command, wait, notification, and AI tasks require a real executor handoff and must be closed with `forge request complete-task --evidence-command "<passing gate>" --evidence-exit-code <observed-exit-code>`. The code must be the value actually observed by the caller; Forge does not fabricate a passing `0` receipt.
`forge request execute-wave --run <run-id> --executor auto --allow-exec --approved-by <operator> --output json` consumes the dependency-ready dispatch frontier and starts its admitted Codex/Agy processes with bounded concurrency. Every mutating assignment must carry a distinct task-scoped worktree claim; Forge correlates wave, workflow revision, task version, context hash and lease into the runtime receipt, deduplicates repeated execution claims, extends the lease for the bounded runtime and keeps it available for explicit validation. The command never treats exit code zero as promotion evidence and never calls `complete-task` automatically.

The primary planning and async-request flows accept the same explicit N:N lane
contract:

```bash
forge plan --goal "Build product" \
  --lane frontend=agy:3 --lane backend=codex:5 \
  --max-parallel-agents 8 --output json

forge request start --goal "Build product" --origin codex \
  --lane frontend=agy:3 --lane backend=codex:5 \
  --max-parallel-agents 8 --output json

forge mcp call forge.run.start --input '{
  "goal": "Build product",
  "origin": "codex",
  "lanes": [
...
the existing generic DAG remains serial; Forge does not guess a team from vague
...
```bash
forge teamwork --goal "Build the product" \
  --lane frontend=agy:3 --lane backend=codex:5 \
  --max-parallel-agents 8 --detached --output json

forge worktree prepare-teamwork --workflow <workflow-id> \
  --repository . --worktree-root ../forge-teamwork-worktrees \
  --branch-prefix forge/product --output json

# Apply the reviewed preparation to that exact workflow.
forge worktree prepare-teamwork --workflow <workflow-id> \
  --repository . --worktree-root ../forge-teamwork-worktrees \
  --branch-prefix forge/product --allow-repository-mutation --output json

forge request execute-wave --run <run-id> --executor auto \
...
forge worktree integrate-dependencies --workflow <workflow-id> \
...
destination, revalidates clean Forge-owned task bindings and frozen Git heads,
...
yields fewer assignments, Forge runs the admitted subset instead of rejecting
the degraded wave.
...
AWS operations are exposed through `forge aws check`, `forge aws inventory`, `forge aws raw` and matching MCP tools. They delegate to `~/plugins/aws-ops/scripts/aws-ops`, use the configured AWS credential-vault defaults and keep secret resolution plus mutation gating outside Forge. This is an opt-in compatibility surface, not a build, startup, installation, backup, or release requirement for Forge Core.
...
`forge` renders `dashboard.guided_cockpit_panel` first, using schema `forge.interactive.guided_cockpit.v1`. The same contract is available through `forge interactive guided-cockpit --output json` and MCP `forge.interactive.guided_cockpit`, with four panes, eight guided steps, current-step detection, preview commands, explicit confirmation gates, rollback hints and safe-action policy.

`forge interactive core-boundary --output json` and MCP `forge.interactive.core_boundary` expose `dashboard.core_boundary_panel` using schema `forge.interactive.core_boundary.v1`. It audits the `forge.core.kernel` Addon, counts domain-specific leaks, lists non-core Addon ownership, surfaces compatibility executors that still route through built-in code, and attaches eight goal3 acceptance gates with evidence commands. The panel is read-only and does not migrate capabilities automatically.
...
`forge interactive improvement-loop --output json` and MCP `forge.interactive.improvement_loop` expose `dashboard.improvement_loop_panel` as the read-only operator view for controlled self-improvement. It combines `forge improve candidates`, structured logs, cost ledger totals, validation failures and context-quality status into a ranked action surface, then points to explicit recover/resume, parallel handoff, normalize-cost, validate and event-policy benchmark/promote commands. The panel never applies a mutation itself; promotion still requires benchmark, validation and explicit approval.
