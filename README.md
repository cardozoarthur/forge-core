...
- replacement-grade CLI readiness through `forge interactive replacement-cli --project-root <project-root>` and MCP `forge.interactive.replacement_cli` with `project_root`, using `forge.interactive.replacement_cli.v1` to aggregate operator home, workflow operations, patch editing UX, action discovery, project-scoped harness/session controls, observability, approvals, milestone evidence, provider-level `provider_readiness` rows and plan-only `provider_wrapper_plans` for approved connected-brain wrappers without launching child CLIs, executing models, mutating project files or counting preparation as release evidence
