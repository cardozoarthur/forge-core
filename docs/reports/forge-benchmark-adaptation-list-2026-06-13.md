...
Open Design, Penpot and n8n are also first-class benchmark references: Open Design informs artifact-centric creative workflows, Penpot informs design tokens/components/layout systems, and n8n informs trigger/action graphs, schedules and node marketplace semantics. n8n is dual-use: it also informs the Core workflow graph and the workflow UI direction.
