...
- `.forge/harness.json` agora aceita `require_lineage_for_exec: true`; nesse modo, `forge harness exec` e MCP `forge.harness.exec` bloqueiam execução real com `harness_exec_blocked_by_project_policy` até receberem `workflow`, `task` e `run`, mantendo CLIs externos dentro de lineage auditável antes de chamar Codex/OpenCode/Gemini/Claude.
