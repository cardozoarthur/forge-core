...
- As ações de patch em `forge interactive command-palette` e as sugestões de autocomplete derivadas delas agora preservam o mesmo `addon_contract`, então menus rápidos e completions mostram que edição de código vem do Addon de desenvolvimento de software, não de uma especialização escondida no Core.
