...
- `forge events observability` e MCP `forge.events.observability` agora expõem `forge.event_observability_index.v1`, um índice derivado da timeline global com filtros por workflow/tenant/node/Addon, buckets por tenant/workflow/node/Addon e agregados de severidade, categoria, duração, retries e waits.
