...
- Detecção inicial de trabalho repetitivo com sinais de executor/IA e recomendação de node determinístico existe em `forge events improvement-policy`, entra no artifact/changelog de `forge improve` e pode ser aplicada por fluxo controlado com rollback/equivalência pendente; falta validar equivalência automaticamente e expandir o fluxo para retries, waits e context pressure.
