...
- `forge milestone manifest --version 0.5 --output json` ainda retorna `promotion_decision.decision=fail`, bloqueado por `replacement_grade_cli` e `experimental_multimodal_runtime`. O harness, o `patch review` e o `patch restore` avançaram a capacidade de CLI de substituição, mas ela continua em `groundwork` até existir navegação rica de diff multi-file na TUI, gestão de provider/session e fluxos reais ponta a ponta de coding/research.
