...
- `forge interactive release-gates`, MCP `forge.interactive.release_gates` e `dashboard.release_gates_panel` adicionam uma superfície operacional read-only para o milestone: mostram decisão de promoção, capacidades bloqueadas, evidência requerida, evidência atual, gaps e próximos comandos por capability sem depender de parsing manual do manifest.
