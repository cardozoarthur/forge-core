...
- As ações de patch em `forge interactive command-palette` agora são geradas a partir das actions da view habilitada `software.patch_workbench`; as sugestões de autocomplete derivadas delas preservam `addon_contract`, `addon_view_id` e `addon_view_action_id`, então menus rápidos e completions mostram que edição de código vem do Addon de desenvolvimento de software, não de uma especialização escondida no Core.
