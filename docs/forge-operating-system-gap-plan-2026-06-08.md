...
- `forge milestone cli-demo` e MCP `forge.milestone.cli_demo` agora incluem fluxo `harness_control` com `interactive_harness_ready`, `headroom_plan_ready`, `session_lifecycle_plan_ready` e token-headroom ativado, além de `forge.milestone.patch_lifecycle_demo.v1`, que cria um repositório fixture isolado no store, executa `patch plan/review/diff/apply/revert/restore` com artifacts reais, registra gates de intake/review/approval/rollback e restaura o target para estado Git limpo sem mutar recursos externos.
