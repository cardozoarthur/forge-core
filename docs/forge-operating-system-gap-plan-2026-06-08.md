...
- `forge milestone evidence-plan`, MCP `forge.milestone.evidence_plan` e painéis que consomem esse contrato agora expõem `manifest_templates` secret-free quando falta configuração de evidência, começando por `.forge/connected-brain-runtimes.json` para `replacement_grade_cli`; o template mostra caminho alvo, JSON de manifesto sem segredo e comandos de validação/coleta antes de qualquer execução real de brain externo.
