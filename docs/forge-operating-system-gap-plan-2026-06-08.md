...
- Exposto `forge harness token-headroom|retrieve-headroom|wrap-plan` e MCP `forge.harness.token_headroom|retrieve_headroom|wrap_plan`, inspirado no Headroom, para reduzir contexto localmente por tipo de conteúdo, persistir blobs reversíveis de headroom em SQLite, recuperar payloads por retrieval ref e gerar planos de wrapper CLI Forge-first para Codex/Claude/Gemini/OpenCode sem interceptar processos automaticamente.
