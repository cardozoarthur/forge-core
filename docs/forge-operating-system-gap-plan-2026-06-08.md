...
- Exposto `forge harness token-headroom|wrap-plan` e MCP `forge.harness.token_headroom|wrap_plan`, inspirado no Headroom, para reduzir contexto localmente por tipo de conteúdo e gerar planos de wrapper CLI Forge-first para Codex/Claude/Gemini/OpenCode sem interceptar processos automaticamente.
...
- Detecção inicial de trabalho repetitivo com sinais de executor/IA e recomendação de node determinístico existe em `forge events improvement-policy`; falta aplicar a recomendação por fluxo controlado com rollback e evidência de equivalência.
