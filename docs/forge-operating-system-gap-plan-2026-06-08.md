...
- `forge addons register-worker` e MCP `forge.addons.register_worker` agora bloqueiam rotação implícita de identidade de workers `signed|trusted`; mudanças de runtime, trust level, signature scheme ou public key exigem `--rotation-approved-by`/`rotation_approved_by` e gravam `forge.addon_runtime_worker_rotation_policy.v1`.
...
- Exposto `forge addons execute-validator` e MCP `forge.addons.execute_validator` como executor auditável para contratos `validator`: padroniza o envelope `forge.addon_validator_dispatch_input.v1`, executa worker registrado, valida a decisão `forge.addon_validator_result.v1`, separa issues de schema de issues relatadas pelo Addon e mantém toda evidência no ledger de dispatch/claim/completion.
- Exposto `forge addons execute-executor` e MCP `forge.addons.execute_executor` como executor auditável para contratos `executor`: padroniza o envelope `forge.addon_executor_dispatch_input.v1`, executa worker registrado, valida o resultado `forge.addon_executor_result.v1`, conta outputs/artifacts/events genéricos e mantém toda evidência no ledger de dispatch/claim/completion.
- Exposto `forge addons execute-handoff` e MCP `forge.addons.execute_handoff` como executor auditável para contratos `handoff`: padroniza o envelope `forge.addon_handoff_dispatch_input.v1`, executa worker registrado, valida o resultado `forge.addon_handoff_result.v1`, exige target/receipt para entregas aceitas ou concluídas, conta artifacts/events genéricos e mantém toda evidência no ledger de dispatch/claim/completion.
- Exposto `forge harness token-headroom|retrieve-headroom|headroom-plan|wrap-plan|install-shims|shim-status|exec|mode` e MCP `forge.harness.token_headroom|retrieve_headroom|headroom_plan|wrap_plan|install_shims|shim_status|exec|mode`, inspirado no Headroom, para reduzir contexto localmente por tipo de conteúdo, persistir blobs reversíveis de headroom em SQLite, recuperar payloads por retrieval ref, gerar planos explícitos de headroom/wrapper CLI Forge-first para Codex/Claude/Gemini/OpenCode, expor `forge.harness.session_lifecycle_plan.v1` com comandos auditáveis de launch/opened/attached/closed, instalar shims PATH não destrutivos quando o usuário preferir a infraestrutura do Forge, auditar precedência/ownership/recursão dos shims, emitir recibos dry-run/guarded de execução e resolver a política efetiva Forge-first sem executar CLIs. O instalador de shims agora resolve o CLI nativo automaticamente via `PATH` quando `real_cmd` não é informado, exclui a própria pasta de shims para evitar recursão e registra origem/status de resolução. `shim-status` lê o script Forge-owned, reporta real CLI/store/Forge binary, detecta shims manuais que tomariam precedência e devolve instruções sem executar processos. `forge harness mode` aplica precedência `observe_only_flag > explicit_flag > env_default > project_config > default_observe_only`, lê `.forge/harness.json` quando existir e expõe status seguro para TUI/MCP antes de `headroom-plan`, `wrap-plan`, `install-shims` ou `exec`. `forge sync executors --shim-dir` agora projeta esse diagnóstico como `forge.executor_harness_status.v1`, `forge_first_ready` e entrypoints Forge-first no relatório de executores, no brain router e nas sessões `/brains`/`/shells`. Execuções reais autorizadas agora aplicam a mesma política de headroom ao stdout/stderr do processo filho, persistem refs reversíveis por stream e mantêm hashes/excertos no recibo `forge.harness.exec_receipt.v1`.
- `forge harness wrap-plan` e MCP `forge.harness.wrap_plan` agora preservam a decisão efetiva de headroom no próprio `launch_command`, incluindo `--token-headroom` ou `--no-token-headroom`, para que comandos copiados mantenham a mesma política resolvida por flag, MCP ou `.forge/harness.json`.
- `forge harness exec` e MCP `forge.harness.exec` agora expõem no receipt top-level `context_budget`, `context_budget_source`, `token_headroom_source` e `require_token_headroom_for_forge_first`, para TUI/MCP/event timeline auditarem a política aplicada sem depender de parsing do `wrapper_plan`.
...
- Exposto `forge interactive harness` e MCP `forge.interactive.harness` como painel dedicado `forge.interactive.harness.v1` para agregar harness mode, doctor, shim status, wrap-plan, `headroom_plan`, `session_lifecycle_plan` de primeira classe e prévia de token-headroom de um brain CLI; o painel também entra em `dashboard.harness_panel` e no `ui_composition_panel` como widget Core para TUI/web/agentes sem instalar shims nem executar CLIs.
- Exposto `forge interactive sessions` e MCP `forge.interactive.sessions` como painel dedicado `forge.interactive.sessions.v1` para readiness de providers/sessões, filtros de provider/lifecycle/readiness, cards de sessão e comandos de history/lifecycle/launch-plan; o painel também entra em `dashboard.sessions_panel` e no `ui_composition_panel` como widget Core sem abrir ou anexar shells implicitamente.
- Exposto `forge interactive patch-workbench` e MCP `forge.interactive.patch_workbench` como painel dedicado `forge.interactive.patch_workbench.v1` para status Git, contagens de arquivos staged/unstaged/untracked, diff stat/check, lanes por arquivo, preview inline, fila multi-file `diff_review_queue`, `edit_intake` com inputs obrigatórios/formulários seguros, `approval_flow` com gates de revisão/aprovação/rollback e comandos de lifecycle `patch plan/review/diff/apply/restore`; o painel também entra no `interactive home` e no `ui_composition_panel` como widget Core para TUI/web sem mutar arquivos.
- O `ui_composition_panel` agora monta widgets de Addons a partir de views habilitadas em todas as superfícies declaradas, incluindo `tui`, e não apenas `ops_console`; isso permite que `forge.addon.software_development` exponha `software.patch_workbench` como widget de Addon ao lado do painel Core de compatibilidade.
- `forge.interactive.patch_workbench.v1` agora carrega `addon_contract` com schema `forge.interactive.patch_addon_contract.v1`, apontando explicitamente para `forge.addon.software_development`, capability `source_code_patch_lifecycle`, permissão `source_code.patch`, view `software.patch_workbench` e contrato `source_code_patch_lifecycle.executor`.
- `AddonViewAction` agora carrega metadados opcionais de operação (`palette_group`, `source_panel`, `description`, `risk_level`, `mutates_workflow`, `command_template`, `keywords`), permitindo que `forge interactive command-palette` gere ações CLI a partir de qualquer view TUI habilitada por Addon. As ações de patch usam esse caminho genérico da view `software.patch_workbench`; palette e autocomplete preservam `addon_contract` com schema `forge.interactive.addon_action_contract.v1`, `addon_view_id`, `addon_view_action_id`, `enabled`, `blocked_reason` e `operation_plan` com schema `forge.interactive.command_palette_action_plan.v1`, então menus rápidos e completions mostram Addon, versão/lifecycle, capability, permissão, status do permission gate, view, action, status executável/diagnóstico e próximos comandos sem transformar edição de código em uma especialização escondida no Core. `forge interactive action-registry`, MCP `forge.interactive.action_registry`, `/actions [query]` e `dashboard.action_registry_panel` agora projetam um registry read-only com filtro estrito, contagens por grupo e totais de ações prontas/bloqueadas/diagnósticas/mutáveis/com aprovação para TUI/web/agentes que não devem depender do layout do command palette. `forge interactive action-invocation`, MCP `forge.interactive.action_invocation` e `/action <action-id>` resolvem uma ação selecionada em comando pronto ou plano diagnóstico com `not_executed=true`, separando seleção/validação de execução real. `forge interactive autocomplete --input "/action <partial>"` agora sugere IDs vindos do action registry, insere `/action <action-id>` e mantém lineage de Addon/action antes de qualquer execução. Ações bloqueadas continuam visíveis para diagnóstico operacional, não expõem comando executável e apontam para inspeção de views do Addon.
...
- Exposto `forge interactive identity --project-root .` e MCP `forge.interactive.identity` como painel dedicado `forge.interactive.identity.v1` para contexto operacional atual, registros de identidade, aliases de canal, memberships e auditoria de tenant; o painel também entra no `interactive home`, no `ui_composition_panel` e na command palette como widget Core read-only para TUI/web/agentes.
...
- Eventos inbound agora podem carregar `identity`/`source_identity` genérico com `scope`, `id` e `label`; `ingest`, `inbox`, `route`, `scan` e o evento auditável `inbound_event_routed` expõem `forge.event_identity_context.v1`, resolvendo alias de canal como Telegram/Discord/Web para a identidade canônica via registry/link sem código de canal no Core.
...
- Adicionada tabela tenant-aware `event_services`, `forge events service-run`, `forge events services --project-root`, MCP `forge.events.service_run` e MCP `forge.events.services`; worker e webhook ingress agora podem rodar como serviços gerenciados bounded com lease persistente, health, status final, operating context por linha e auditoria em `global_events`. O worker renova heartbeat/lease por ciclo e aceita `--stop-file`/`stop_file` para shutdown cooperativo com status final `stopped`; o webhook ingress persiste heartbeats de progresso enquanto escuta/aguarda requests, aceita o mesmo stop-file entre requests, grava `webhook_report` e health final de requests/ingest/route/stop.
...
- `events services --project-root`, `events services-recover` e MCP `forge.events.services|services_recover` agora exigem `context:read` em `tenant_policy_mode: enforce`, filtram organization/brand/product e impedem que serviços/leases de outro tenant apareçam ou sejam recuperados. `services-recover` marca serviços `running` com lease vencida como `stale`, preservando o payload de health e adicionando marcador `forge.event_service_recovery_marker.v1` com origem, lease e heartbeat observados.
...
- `forge interactive home --project-root <path>` e MCP `forge.interactive.home` com `project_root` agora expõem painéis estruturados para foco de workflows, task board operacional, release gates project-root-aware, schedule worker, timeline global de eventos, cost ledger e contexto/memória, além do resumo textual da TUI; isso dá ao operador humano e a agentes externos a mesma visão de lanes de workflow, handoffs prontos, checkpoints, interações humanas, artefatos, custos, waits/schedules e memória sem abrir cada workflow manualmente ou depender do cwd.
...
- `forge interactive home --project-root <path> --output json` inclui `dashboard.harness_panel`, `dashboard.sessions_panel`, `dashboard.harness_mode_panel` e `dashboard.harness_doctor_panel`, então o operador vê o centro de harness/headroom, o centro de sessões, o modo Forge-first e a readiness completa do harness no painel inicial junto dos demais painéis operacionais.
...
- `forge interactive harness`, MCP `forge.interactive.harness` e `dashboard.harness_panel` adicionam a superfície operacional consolidada de harness/headroom para TUI/web/agentes: ela agrega modo efetivo, doctor, status de shim, plano de wrapper, `headroom_plan`, `session_lifecycle_plan` de primeira classe e prévia de token-headroom, com comandos granulares para `doctor`, `shim-status`, `wrap-plan`, `headroom-plan`, `install-shims`, `exec`, `sessions` e `sync`.
- `forge interactive sessions`, MCP `forge.interactive.sessions` e `dashboard.sessions_panel` adicionam a superfície operacional consolidada de sessões/brains para TUI/web/agentes: ela agrega providers, sessões, estado de lifecycle, histórico de shell e comandos de próximo lifecycle sem executar abertura/anexo/fechamento automaticamente.
...
- `forge interactive patch-workbench`, MCP `forge.interactive.patch_workbench` e `dashboard.patch_workbench_panel` adicionam a superfície operacional de file editing/diff review para TUI/web/agentes: ela mostra arquivos alterados, filtra arquivos internos do store Forge, inclui `diff_preview` inline e bounded com linhas classificadas, inclui `diff_review_queue` multi-file com hunks/adições/remoções por arquivo, resume diff check/stat, expõe `edit_intake` (`forge.interactive.patch_edit_intake.v1`) com campos ausentes, formulários e templates seguros para `plan/review/diff/apply/revert/restore`, expõe `operation_plan` (`forge.interactive.patch_operation_plan.v1`) com sequência de lifecycle, dependências, flags de mutação e gates de aprovação humana, expõe `approval_flow` com gate atual e gates de revisão/aprovação/rollback, e aponta para comandos permission-gated do lifecycle de patch.
- `forge milestone cli-demo` e MCP `forge.milestone.cli_demo` agora incluem fluxo `harness_control` com `interactive_harness_ready`, `headroom_plan_ready`, `session_lifecycle_plan_ready` e token-headroom ativado, `forge.milestone.patch_lifecycle_demo.v1`, que cria um repositório fixture isolado no store, executa `patch plan/review/diff/apply/revert/restore` com artifacts reais, registra gates de intake/review/approval/rollback e restaura o target para estado Git limpo sem mutar recursos externos, `forge.milestone.executor_project_demo.v1`, que prova execução governada por harness em projeto isolado, e `forge.milestone.brain_handoff_demo.v1`, que prova contexto, node-brain routing, task handoff, plano de shell e lifecycle audit-only para Codex sem executar child CLI/modelo.
...
- `forge sessions lifecycle`, MCP `forge.session.lifecycle` e `/sessions lifecycle` adicionam controle audit-only de lifecycle para shell sessions conhecidas, registrando estados `opened`, `attached`, `detached`, `closed`, `failed` ou `abandoned` no timeline global sem executar child process. O lifecycle agora é ordenado: recibos carregam `previous_state`, `lifecycle_sequence` e `transition`, transições inválidas são recusadas antes do evento global, e `forge sessions`/MCP `forge.sessions` expõem `lifecycle_policy.allowed_next_states` com comandos de próxima transição. `forge sessions` também filtra por `--provider`, `--state` e `--readiness`, com equivalentes MCP `provider_id`, `lifecycle_state` e `readiness`, para UI/agentes trabalharem em uma lane de provider ou lifecycle sem filtrar todo o JSON no cliente. `forge.brain_session_operation_plan.v1` agora acompanha cada sessão com a ação recomendada, lineage completo, requisitos de contexto/handoff/heartbeat e comandos concretos de history, launch-plan, record-plan e lifecycle. `forge sessions history --session <id>`, MCP `forge.session.history` e `/sessions history` retornam o histórico cronológico de uma sessão específica, separando eventos `shell_launch_planned` e `brain_session_lifecycle`, contadores, estado atual, policy e próximos comandos auditáveis.
- `forge interactive release-gates --project-root <path>`, MCP `forge.interactive.release_gates` e `dashboard.release_gates_panel` adicionam uma superfície operacional read-only para o milestone: mostram decisão de promoção, capacidades bloqueadas, evidência requerida, evidência atual, gaps, próximos comandos e o `evidence_plan` por capability, incluindo templates secret-free de manifests faltantes sem depender de parsing manual do manifest.
- `forge milestone evidence-plan`, MCP `forge.milestone.evidence_plan` e painéis que consomem esse contrato agora expõem `manifest_templates` secret-free quando falta configuração de evidência, incluindo `.forge/connected-brain-runtimes.json` para `replacement_grade_cli` e `.forge/multimodal.json`/`.forge/multimodal-runtimes.json` para `experimental_multimodal_runtime`; cada template mostra caminho alvo, JSON de manifesto sem segredo e comandos de validação/coleta antes de qualquer execução real de brain externo ou runtime multimodal.
- `forge milestone manifest --version 0.5 --output json` ainda retorna `promotion_decision.decision=fail`, bloqueado por `replacement_grade_cli` e `experimental_multimodal_runtime`. O `harness doctor`, o harness center com `session_lifecycle_plan`, o `patch review`, o `patch diff`, o `patch restore`, o `interactive patch-workbench` com `approval_flow` explícito, `edit_intake`, `operation_plan`, `diff_preview` inline e `diff_review_queue` multi-file, o `patch_lifecycle_demo`, o `executor_project_demo`, o `brain_handoff_demo`, o `interactive permissions` e o `operation_plan` de sessão avançaram a capacidade de CLI de substituição, mas ela continua em `groundwork` até existir UX mais completa de permissão/file editing e fluxos reais ponta a ponta de coding/research com execução real de cérebro externo.
...
Status: inbox global tenant-aware, roteamentos `start_workflow`, `continue_workflow`, `modify_workflow`, `pause_workflow`, `resume_workflow` e `complete_workflow`, descoberta declarativa de `event_adapters` por Addon com `permission_gate`, policy gate inicial em `events route`, worker single-pass `events scan`, loop local bounded `events worker`, contrato plan-only `events service-plan`/MCP para serviço gerenciado com lease/backoff/health/shutdown, execução bounded de worker via `events service-run`/MCP com `event_services` tenant-aware persistido e lease/heartbeat renovados por ciclo, execução bounded de webhook ingress via `events service-run --kind webhook_ingress`/MCP com heartbeats de progresso, `webhook_report` e health persistidos, listagem tenant-aware `events services --project-root`/MCP, recuperação explícita de serviços stale por `events services-recover`/MCP, recuperação opt-in de leases `worker` stale durante `runtime-reconcile`/`runtime-daemon`, supervisor bounded `events service-supervise`/MCP sobre `service-run` com restart/backoff/stop-file/health agregado, reconciler inicial `events runtime-reconcile`/MCP consumindo `forge.registry_workflow_runtime.v1`, registry/inbox/event_services filtrados por tenant, leases e schedules para recomendar/executar worker supervisor ou reidratar cron/wait_until due por `scan_due`, daemon `events runtime-daemon`/MCP com lease própria em `event_services`, modo bounded ou contínuo, stop-file/idle-exit cooperativos e retenção limitada de ciclos, listener bounded `events webhook-ingress` para POST HTTP real com HMAC-SHA256 opcional por segredo em env, emissor genérico `events emit` para `http://`/`https://` egress com HMAC-SHA256 e Bearer por env ou credential-vault, e adapters Telegram de mensagem/documento/relatório com dry-run, entrega Bot API controlada por env ou credential-vault e artifact de evidência por workflow implementados. `events ingest --project-root`, `events inbox --project-root`, `events services --project-root` e MCP equivalentes agora persistem/projetam operating context em `event_inbox`/`event_services`; em `tenant_policy_mode: enforce`, `events inbox`, `events scan`, `events services`, `events services-recover` e `events runtime-reconcile` exigem `context:read` e filtram organization/brand/product antes de expor, recuperar ou consumir workflows, eventos pendentes e estado operacional. `continue_workflow` já cobre anexar evidência, checkpoint, resposta humana, conclusão de task pronta e drive de run; `webhook-ingress` já cobre POST JSON -> validação HMAC opcional -> inbox tenant-aware -> route opcional via adapter policy; `events emit` já cobre validação de adapter, permission gate, allowlist de host, HMAC/Bearer/bot_token por env ou credential-vault, tenant policy `workflow:deliver` antes do transporte com workflow alvo ou pelo operating context do projeto sem workflow alvo, envelope tipado, relatório de entrega, auditoria persistida na timeline global e `workflow_artifact` quando há workflow alvo.
...
- Addons ainda precisam de workers WASM assinados reais, políticas mais amplas de trust/rotação/revogação e promoção administrável de runtime externo; rotação explícita de identidade para worker `signed|trusted`, contratos declarativos, preflight de dispatch, ledger persistente, registry de workers, claim/completion, executor `local_process`, executor `external_api` HTTP/HTTPS com auth/secret injection, `execute-planner`, `execute-validator`, `execute-executor`, `execute-handoff` e processamento seguro do Core já existem como manifesto/catalogação/fila/execução auditada.
