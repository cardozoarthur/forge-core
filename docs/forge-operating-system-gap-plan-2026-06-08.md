...
- Addons ainda precisam executar `executor` e `handoff` com o mesmo nível de auditoria especializada dos planners/validators e precisam de workers WASM assinados reais; contratos declarativos, preflight de dispatch, ledger persistente, registry de workers, claim/completion, executor `local_process`, executor `external_api` HTTP/HTTPS com auth/secret injection, `execute-planner`, `execute-validator` e processamento seguro do Core já existem como manifesto/catalogação/fila/execução auditada.
