...
`forge interactive release-gates --project-root <project-root>` and MCP `forge.interactive.release_gates`
...
Forge separates five concepts that must not be collapsed into one path:
...
The persistent schemas are `forge.worktree.record.v1`, `forge.worktree.binding.v1` and `forge.worktree.sandbox_lifecycle.v1`; policy/action outputs add `forge.worktree.config.v1`, `forge.worktree.modification_guard.v1`, `forge.worktree.predecessor_task.v1`, `forge.worktree.sandbox_plan.v1` and `forge.worktree.sandbox_receipt.v1`. Bindings are stored in the central SQLite database, increment workflow revision, and emit lineage events. Resolution is deterministic: a task-specific binding wins; otherwise Forge uses the workflow-level binding. Context and executor handoff carry `repository_root`, `worktree_root`, branch, HEAD, dirty state, manifest status/path/SHA-256, approval state/approved hash, effective guardrails, sandbox configuration, settings and bindings.

Use a single absolute `--store` path outside disposable worktrees. Every related command and detached run must use that same central store. Sandbox launch exports its resolved host path as `FORGE_STORE_PATH`; a relative default tied to cwd must not become a second worktree-local authority. Process payloads can use that host locator. Bubblewrap payloads receive it only as lineage metadata because the external SQLite store is not mounted in the guest.

The lifecycle surfaces are:

```bash
forge --store <absolute-store> worktree discover --repository <repository-root> --output json
forge --store <absolute-store> worktree create --repository <repository-root> --path <worktree-root> --branch <branch> --start-point <git-ref> --allow-repository-mutation --output json
forge --store <absolute-store> worktree register --path <existing-worktree> --output json
forge --store <absolute-store> worktree bind --worktree <id-or-path> --workflow <workflow-id> --task <task-id> --origin forge_cli --output json
forge --store <absolute-store> worktree init --worktree <id-or-path> --allow-worktree-write --output json
forge --store <absolute-store> worktree approve-config --worktree <id-or-path> --allow-guardrail-update --approved-by <operator-id> --origin forge_cli --output json
forge --store <absolute-store> worktree guard check --worktree <id-or-path> --operation modify --path <relative-path> --reason <objective> --workflow <workflow-id> --task <task-id> --output json
forge --store <absolute-store> worktree guard create-predecessor --worktree <id-or-path> --workflow <workflow-id> --task <task-id> --path <relative-path> --goal <objective> --allow-workflow-mutation --approved-by <operator-id> --origin forge_cli --output json
forge --store <absolute-store> worktree list --workflow <workflow-id> --output json
forge --store <absolute-store> worktree inspect --worktree <id-or-path> --output json
```

`create` is the only surface here that mutates Git and requires `--allow-repository-mutation`. `init` writes `.forge/worktree.toml` plus Forge-owned internal sandbox directories, requires `--allow-worktree-write`, and records the generated config hash as approved. An existing manifest is not replaced without `--force`. `approve-config` requires `--allow-guardrail-update` plus `--approved-by` before central state accepts a changed hash. Approver/origin values are audit provenance and are not locally authenticated identities. The current CLI has no worktree removal command, so registration does not authorize deletion of external or Forge-created checkouts.

The manifest schema is `forge.worktree.config.v1`. It controls clean/detached/branch policies, executable-basename allowlists, modifiable and protected path scopes, required workflow binding, timeout, maximum captured output, enabled purposes, sandbox runtime, working directory, network policy, explicit environment and inherited environment names. The generated defaults are `modifiable_paths=["."]` and `protected_paths=[".git/", ".forge/worktree.toml"]`. Protected scopes take precedence. The manifest content hash participates in context and receipts; modification checks and sandbox plans require the current hash to be approved, and execution is rejected if it changes between planning and launch.
...
Plans distinguish host paths (`worktree_root`, `sandbox_root`, `working_directory`) from execution paths (`runtime_worktree_root`, `runtime_sandbox_root`, `runtime_working_directory`). Under Bubblewrap the worktree is `/workspace`, the internal sandbox remains at its relative path below `/workspace`, HOME is the bound `sandbox/home` at `/home/forge`, and tmp is the bound `sandbox/tmp` at `/tmp`. `FORGE_WORKTREE_ROOT` and `FORGE_SANDBOX_ROOT` use guest paths; `FORGE_STORE_PATH` remains an inaccessible host locator and `forge_store_path_mounted=false` makes that machine-readable. Nested Forge mutations must run outside Bubblewrap through the parent workflow.
