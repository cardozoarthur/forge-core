...
`forge harness headroom-plan --executor codex|claude|gemini|opencode --project-root <project-root> --output json` and MCP `forge.harness.headroom_plan` expose `forge.harness.headroom_plan.v1`, a read-only plan that resolves the effective Forge-first mode, context budget, token-headroom source, project `require_token_headroom_for_forge_first` policy, wrapper env, compression pipeline, reserve strategy, retrieval policy and next commands before a brain CLI receives large logs, tool output or child stdout. The report is intentionally separate from execution and shim installation so TUI, web and agent clients can inspect token/headroom readiness without mutating workflow state or launching external CLIs.
