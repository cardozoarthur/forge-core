...
`forge events observability --workflow <workflow-id> --node <node-ref> --addon <addon-id> --output json` exposes `forge.event_observability_index.v1`. It derives a normalized index from `forge.event_timeline.v1`, filters by workflow/organization/brand/product/node/Addons, and returns tenant, workflow, node and Addon buckets with severity/category counts plus duration, retry and wait aggregates. This gives the Cost OS, dashboard renderers and improvement loop a queryable event-observability surface before a dedicated physical observability table exists.
