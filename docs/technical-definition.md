...
This fixture-only result is not promotion evidence for real image/audio/video/3D execution. Promotion requires guard-approved runtime access and real benchmark/demo artifacts that prove camera, microphone, screen, input, filesystem and network access remain blocked unless the operator opts in through Forge policy.
