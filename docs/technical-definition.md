...
`forge interactive home --output json` and MCP `forge.interactive.home` expose the operator home surface as structured state, not only text. The dashboard now includes `workflow_focus`, `harness_mode_panel`, `digital_twin_panel`, `task_board_panel`, `schedule_panel`, `event_panel`, `cost_panel`, `context_memory_panel` and `addon_renderer_panel`, allowing a TUI, web console or external agent to render current workflow priorities, effective Forge-first harness mode/source/project-config status, operational digital twin state, task lanes, ready handoffs, checkpoint resume candidates, pending human waits, attached artifact counts, due waits/schedules, global event timeline, cost totals, memory/context posture and safe Addon UI renderer families from one call. The `harness_mode_panel` uses schema `forge.harness.mode.v1`, matching CLI `forge harness mode`, MCP `forge.harness.mode` and interactive slash command `/harness`; the `digital_twin_panel` reuses `forge.ops.operational_digital_twin.v1` so the TUI sees what is happening, done, remaining, validated, rejected and awaiting approval per workflow; the `task_board_panel` uses schema `forge.interactive.task_board.v1` and carries lane-level next-action commands plus operable `task_cards` with task id, title, status, executor, human-interaction state, handoff readiness, context action, checkpoint id/state and direct inspect/context/handoff/interaction commands. The same task-board contract is exposed directly as `forge interactive task-board --output json` and MCP `forge.interactive.task_board`, so agents and future TUI/web clients can render the board without requesting the full home dashboard. The human renderer prints matching sections so non-JSON terminal use remains useful.
